...
const BlinkingCursor: React.FC = () => {
  ...
  return (
    <Animated.View
      style={[{ width: 2, height: 18, backgroundColor: palette.indigo, marginLeft: 2, borderRadius: 1 }, style]}
    />
  );
};
...
export const AnalysisContentDisplay: React.FC<AnalysisContentDisplayProps> = ({
  ...
  const renderTextWithTimecodes = (text: string) => {
    ...
    while ((match = regex.exec(text)) !== null) {
      if (match.index > lastIndex) {
        parts.push(<Text key={`t-${key++}`} style={{ color: textColor }}>{text.slice(lastIndex, match.index)}</Text>);
      }
      ...
      parts.push(
        ...
        >
          <Ionicons name="play" size={10} color="#60A5FA" style={{ marginRight: 3 }} />
          <Text style={localStyles.timecodeText}>{match[0].replace(/[\[\]]/g, '')}</Text>
        </Pressable>
      );
      lastIndex = match.index + match[0].length;
    }
    if (lastIndex < text.length) {
      parts.push(<Text key={`t-${key++}`} style={{ color: textColor }}>{text.slice(lastIndex)}</Text>);
    }
  ...
  const renderEpistemicCallout = (marker: string) => {
    ...
    return (
      <Animated.View
        ...
        style={[localStyles.epistemicCallout, {
  ...
  const rules = useMemo(() => ({
    heading1: (node: any, children: any, _parent: any, styles: any) => {
      const rawText = node.children?.map((c: any) => c.content || '').join('') || '';
      const emoji = getHeaderEmoji(rawText);
      return (
        <View key={node.key}>
          <Text style={styles.heading1}>{emoji}  {children}</Text>
        </View>
      );
    },
    heading2: (node: any, children: any, _parent: any, styles: any) => {
      ...
      const emoji = getHeaderEmoji(rawText);
      return (
        <Text key={node.key} style={styles.heading2}>{emoji}  {children}</Text>
      );
    },
    text: (node: any, _children: any, _parent: any, styles: any) => {
            ...
            <Text key={node.key} style={styles.text}>
              {parts.map((part: string, idx: number) => {
                if (EPISTEMIC_MARKERS[part.toUpperCase()]) {
                  return <React.Fragment key={`ep-${idx}`}>{renderEpistemicCallout(part)}</React.Fragment>;
                }
                return <Text key={`txt-${idx}`} style={{ color: textColor }}>{part}</Text>;
              })}
            </Text>
          );
        }
      ...
      return <Text key={node.key} style={[styles.text, { color: textColor }]}>{text}</Text>;
    },
  ...
  if (error) {
    return (
      <Animated.View style={localStyles.stateContainer} entering={FadeInDown.duration(400).springify()}>
        <Ionicons name="alert-circle-outline" size={48} color={colors.accentError} />
        <Text style={[localStyles.stateText, { color: colors.textSecondary }]}>{error}</Text>
        {onRetry && (
          <Pressable
            onPress={onRetry}
            style={[localStyles.retryBtn, { backgroundColor: colors.accentPrimary }]}
  ...
  if (showEmptyState || !displayText) {
    return (
      <Animated.View style={localStyles.stateContainer} entering={FadeIn.duration(400)}>
    ...
    <Animated.View style={localStyles.wrapper} entering={FadeIn.duration(400)}>
      ...
      <Animated.View style={[copyAnimStyle]} entering={FadeInDown.delay(200).duration(300)}>
        ...
        >
          ...
          <Text style={[localStyles.copyLabel, { color: colors.textSecondary }]}>Copier</Text>
        </Pressable>
      </Animated.View>
