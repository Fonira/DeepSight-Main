 ...
 *
 * Port React Native de frontend/src/hooks/useTimeOfDay.ts.
 *
 ...
 *
 * Recalcule toutes les 5 minutes via setInterval. Détecte reduce-motion via
 * AccessibilityInfo (RN n'a pas window.matchMedia).
 *
 * Utilisation :
 *   const { phase, ambientPreset, prefersReducedMotion } = useTimeOfDay();
 */
 ...
 * soient bien distinctes visuellement, conformément au brief web.
 */
...
function readNow(): NowSnapshot {
  const now = new Date();
  return { hour: now.getHours(), dayOfWeek: now.getDay() };
}

export function useTimeOfDay(): TimeOfDayValue {
  const [snapshot, setSnapshot] = useState<NowSnapshot>(() => readNow());
  const [prefersReducedMotion, setPrefersReducedMotion] =
    useState<boolean>(false);

  // Recalcule l'heure toutes les 5 minutes
  useEffect(() => {
  ...
  // Détection reduce-motion (RN n'a pas window.matchMedia)
  useEffect(() => {
    let mounted = true;
