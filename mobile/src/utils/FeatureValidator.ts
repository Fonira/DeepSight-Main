      ...
      try {
        const { result: response, duration } = await measureTime(() =>
          fetch(`${API_BASE_URL}/api/auth/google/login`, {
            ...
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({
              redirect_uri: "deepsight://auth/callback",
              platform: "mobile",
            }),
          }),
        );
      ...
      try {
        ...
        if (
          ...
          return {
            ...
            status: "warning",
            message:
              "Login a réussi avec des identifiants test - vérifier la sécurité",
            duration,
          };
        } else {
          return {
...
export class FeatureValidator {
  ...
  constructor() {
    ...
    this.validations = [...authValidations, ...apiValidations];
  }
  ...
  async validateCategory(
    category: FeatureValidation["category"],
  ): Promise<ValidationResult[]> {
    log(`Validation de la catégorie: ${category}`);
    const categoryValidations = this.validations.filter(
  ...
  printReport(): void {
    ...
    console.log("----------------------------------------");
    console.log(
      `Total: ${summary.total} | ✅ ${summary.success} | ⚠️ ${summary.warnings} | ❌ ${summary.errors}`,
    );
    console.log("========================================\n");
  }
}
...
export const runStartupValidation = async (): Promise<void> => {
  if (__DEV__) {
    console.log(
      "[FeatureValidator] Exécution de la validation au démarrage...",
    );
