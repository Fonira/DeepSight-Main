...
export const AnalysisScreen: React.FC = () => {
  ...
  const loadCompletedAnalysis = useCallback(
    async (summaryIdToLoad: string) => {
      try {
        ...
        // Track pour In-App Rating (d�clenche apr�s 3+ analyses)
  ...
  const handleSendMessage = async (customMessage?: string) => {
    ...
    try {
      ...
      if (err?.status === 402) {
        errorMessage =
          language === "fr"
            ? "Quota de chat d�pass�. Passez � un plan sup�rieur."
            : "Chat quota exceeded. Please upgrade your plan.";
      } else if (err?.status === 429) {
        errorMessage =
          language === "fr"
            ? "Trop de requ�tes. Veuillez patienter un moment."
            : "Too many requests. Please wait a moment.";
      } else if (err?.code === "TIMEOUT") {
        errorMessage =
          language === "fr"
            ? "�a prend plus de temps que pr�vu � r�essayez."
            : "Taking longer than expected � try again.";
      } else if (err?.code === "NETWORK_ERROR") {
        errorMessage =
          language === "fr"
            ? "Connexion perdue � on r�essaie dans un instant"
  ...
  const tabs: TabItem[] = [
    ...
    {
      id: "study",
      label: language === "fr" ? "R�viser" : "Study",
      icon: "school-outline",
    },
  ];
  ...
  return (
    <View style={[styles.container, { backgroundColor: "transparent" }]}>
        ...
        >
          ...
          >
            ...
            <View style={styles.badgesRow}>
              ...
              <Badge label={summary?.category || "G�n�ral"} variant="default" />
              {summary?.language && (
                <Badge
      ...
      {activeTab === "study" && (
        <View style={{ flex: 1 }}>
          {/* Segmented Control: Chat IA / Outils d'�tude */}
          {!activeStudyTool && (
            <View
              style={[
                ...
                planUsage.chat_messages_today >= planLimits.chatDailyLimit && (
                  <View style={styles.chatLimitWarning}>
                    <Text style={styles.chatLimitText}>
                      {language === "fr"
                        ? "Limite atteinte. Passez au plan sup�rieur pour continuer."
