  ...
  const loadAnalyses = useCallback(
    async (pageNum: number = 1, reset: boolean = false) => {
      ...
      try {
        ...
        if (__DEV__) {
          console.log(
            `[HistoryScreen] Loaded ${response.items.length} items, hasMore=${response.hasMore}`,
          );
          if (response.items.length > 0) {
            const first = response.items[0];
            console.log(
              `[HistoryScreen] First item: id=${first.id}, platform=${first.platform}, video_url=${first.video_url?.slice(0, 40)}, title=${first.title?.slice(0, 30)}`,
            );
          }
        }
    ...
    async (pageNum: number = 1, reset: boolean = false) => {
      ...
      try {
        ...
        if (reset || pageNum === 1) {
          ...
          setPlaylists((prev) => [
            ...prev,
            ...(response.items as unknown as PlaylistHistoryItem[]),
          ]);
        }
  ...
  const renderEmpty = () => {
    ...
    return (
      <EmptyState
        ...
        actionLabel={
          !showFavoritesOnly && !searchQuery ? t.dashboard.analyze : undefined
        }
        onAction={
      ...
      >
          ...
          {(item.thumbnail_urls || []).slice(0, 4).map((url, idx) => (
            <Image
              ...
              style={[
                ...
                (item.thumbnail_urls || []).length === 1 &&
                  styles.playlistThumbFull,
              ]}
              contentFit="cover"
            />
          ))}
        ...
        <View style={styles.playlistInfo}>
          ...
          <View style={styles.playlistMeta}>
            ...
            />
            <Text
              style={[styles.playlistMetaText, { color: colors.textSecondary }]}
            >
              {item.video_count} vidéo{item.video_count > 1 ? "s" : ""}
            </Text>
            <Text style={[styles.playlistMetaDot, { color: colors.textMuted }]}>
              •
            </Text>
            <Text
              style={[styles.playlistMetaText, { color: colors.textMuted }]}
            >
        ...
        >
          ...
          <Text
            style={[
              ...
              {
                color:
                  activeTab === "videos" ? "#FFFFFF" : colors.textSecondary,
              },
            ]}
          >
            Vidéos
        ...
        >
          ...
          <Text
            style={[
              ...
              {
                color:
                  activeTab === "playlists" ? "#FFFFFF" : colors.textSecondary,
              },
            ]}
          >
            Playlists
                ...
                <View style={styles.filterChips}>
                  ...
                  {modes.map((mode) => (
                    <Pressable
                      ...
                      onPress={() =>
                        setSelectedMode(
                          selectedMode === mode.key ? null : mode.key,
                        )
                      }
                    >
                      <Text
          ...
          {!isLoading && analyses.length > 0 && (
            <View style={styles.resultsInfo}>
              ...
              <View style={styles.viewModeIndicator}>
                ...
                />
                <Text
                  style={[styles.viewModeText, { color: colors.textMuted }]}
                >
                  {viewMode === "list"
                    ? t.history.listView
                    : t.history.gridView}
