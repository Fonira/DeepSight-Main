...
describe('API Client', () => {
  beforeEach(() => {
    ...
    (tokenStorage.getAccessToken as jest.Mock).mockResolvedValue('test-access-token');
  });

  describe('Auth Token Headers', () => {
    it('should include Authorization header with Bearer token', async () => {
      mockFetchResponse({ id: 1, username: 'test', email: 'test@test.com', plan: 'free' });
    ...
    it('should use TokenManager token when available', async () => {
      (tokenManager.getValidToken as jest.Mock).mockResolvedValue('managed-token');
      mockFetchResponse({ id: 1, username: 'test', email: 'test@test.com', plan: 'free' });
  ...
  describe('ApiError Class', () => {
    it('should have correct properties', () => {
      const error = new ApiError('Test error', 400, 'BAD_REQUEST', 'Bad request');
    ...
    it('should detect verification via detail field', () => {
      const error = new ApiError('Not verified', 403, undefined, 'EMAIL_NOT_VERIFIED');
      expect(error.isEmailNotVerified).toBe(true);
    });
  });
  ...
  describe('Token Refresh on 401', () => {
    it('should attempt refresh when receiving 401', async () => {
      ...
      expect(tokenStorage.setTokens).toHaveBeenCalledWith('refreshed-token', 'new-refresh');
    });
  });
  ...
  describe('Google OAuth', () => {
    it('should exchange Google token correctly', async () => {
      const response = {
        ...
        user: { id: 1, username: 'googleuser', email: 'google@test.com', plan: 'free' },
      };
      ...
      expect(tokenStorage.setTokens).toHaveBeenCalledWith('session-token', 'session-refresh');
    });
  });
