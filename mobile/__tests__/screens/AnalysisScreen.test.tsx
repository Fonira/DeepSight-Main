  ...
  useLanguage: () => ({
    ...
    t: {
      ...
      analysis: {
        ...
        notesPlaceholder: "Ajoutez vos notes...",
        noNotes: "Aucune note",
...
jest.mock("../../src/services/api", () => ({
  videoApi: {
    ...
    getReliability: jest.fn().mockResolvedValue({
      overall_score: 0.8,
      confidence: "high",
      factors: [],
      recommendations: [],
...
jest.mock("../../src/components/study", () => ({
  FlashcardsComponent: () => null,
  QuizComponent: () => null,
}));
jest.mock("../../src/components/factcheck", () => ({
  FactCheckButton: () => null,
}));
jest.mock("../../src/components/enrichment", () => ({
  WebEnrichment: () => null,
}));
jest.mock("../../src/components/academic", () => ({
  AcademicSourcesSection: () => null,
}));
jest.mock("../../src/components/tournesol", () => ({
  TournesolWidget: () => null,
}));
jest.mock("../../src/components/analysis/AnalysisValueDisplay", () => ({
  AnalysisValueDisplay: () => null,
}));
jest.mock("../../src/components/analysis/AnalysisContentDisplay", () => {
  const React = require("react");
  const { View, Text } = require("react-native");
  return {
    AnalysisContentDisplay: ({ content }: any) => (
      <View testID="content-display">
        <Text>{content}</Text>
      </View>
    ),
  };
});
jest.mock("../../src/components/audio/TTSPlayer", () => ({
  TTSPlayer: () => null,
}));
...
jest.mock("../../src/components/upgrade", () => ({
...
describe("AnalysisScreen", () => {
  beforeEach(() => {
    ...
    mockGetEnrichedConcepts.mockResolvedValue({
      concepts: [{ name: "Machine Learning", definition: "A subset of AI" }],
    });
