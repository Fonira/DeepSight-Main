  ...
  const handleQuickChat = useCallback(async () => {
    ...
    try {
      ...
      router.push({
        ...
        params: { id: String(result.summary_id), backTo: 'study', initialTab: '1', quickChat: 'true' },
      } as any);
    } catch (err: any) {
      Alert.alert('Erreur', err?.message || 'Impossible de préparer le Quick Chat.');
    } finally {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <View style={[styles.segmentedControl, { backgroundColor: colors.bgSecondary, borderColor: colors.border }]}>
          <Pressable
            style={[
              styles.segmentBtn,
              activeSubTab === 'chat' && [styles.segmentActive, { backgroundColor: palette.indigo + '20' }],
            ]}
            onPress={() => { setActiveSubTab('chat'); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); }}
          >
            <Ionicons
              name="chatbubbles-outline"
              size={16}
              color={activeSubTab === 'chat' ? palette.indigo : colors.textTertiary}
            />
            <Text style={[
              styles.segmentText,
              { color: activeSubTab === 'chat' ? palette.indigo : colors.textTertiary },
          ...
          <Pressable
            style={[
              styles.segmentBtn,
              activeSubTab === 'reviser' && [styles.segmentActive, { backgroundColor: palette.indigo + '20' }],
            ]}
            onPress={() => { setActiveSubTab('reviser'); Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light); }}
          >
            <Ionicons
              name="school-outline"
              size={16}
              color={activeSubTab === 'reviser' ? palette.indigo : colors.textTertiary}
            />
            <Text style={[
              styles.segmentText,
              { color: activeSubTab === 'reviser' ? palette.indigo : colors.textTertiary },
          ...
          <>
            ...
            <Animated.View entering={FadeInDown.delay(50).duration(250)}>
              <View style={[styles.quickChatBox, { backgroundColor: colors.bgElevated, borderColor: colors.border }]}>
                <View style={styles.quickChatHeader}>
                  ...
                  <Text style={[styles.quickChatLabel, { color: colors.textPrimary }]}>Quick Chat</Text>
                  <Text style={[styles.quickChatBadge, { backgroundColor: palette.green + '20', color: palette.green }]}>0 crédit</Text>
                </View>
                <Text style={[styles.quickChatDesc, { color: colors.textTertiary }]}>
                  Colle un lien YouTube ou TikTok pour chatter directement avec la vidéo
                </Text>
            ...
            {loading && (
              <View style={styles.loadingBox}>
                <DeepSightSpinner size="sm" speed="normal" label="Chargement..." />
              </View>
            )}

            {/* Empty state */}
            {!loading && summaries.length === 0 && (
              <Animated.View entering={FadeInDown.delay(100).duration(300)} style={styles.emptyState}>
            ...
            {summaries.length > 0 && (
              <Animated.View entering={FadeInDown.delay(100).duration(300)}>
                ...
                {summaries.map((summary) => {
                  ...
                  const rawThumb = summary.thumbnail || summary.videoInfo?.thumbnail;
                  const thumbUrl = rawThumb
                    || (plat === 'youtube' && summary.videoId ? `https://img.youtube.com/vi/${summary.videoId}/mqdefault.jpg` : null);
                  return (
                    <Pressable
                      key={summary.id}
                      style={[styles.chatCard, { backgroundColor: colors.bgElevated, borderColor: colors.border }]}
                      onPress={() => {
                        Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
                        router.push({
                          pathname: '/(tabs)/analysis/[id]',
                          params: { id: summary.id, backTo: 'study', initialTab: '1' },
                        } as any);
                      }}
                    >
                      {/* Miniature vidéo */}
                      <View style={styles.thumbWrapper}>
                        {thumbUrl ? (
                          ...
                          <View style={[styles.thumbPlaceholder, { backgroundColor: plat === 'tiktok' ? '#111' : colors.bgSecondary }]}>
                            <Ionicons name={plat === 'tiktok' ? 'musical-notes-outline' : 'videocam-outline'} size={20} color={plat === 'tiktok' ? '#fff' : colors.textMuted} />
                          </View>
                        )}
                        {/* Badge plateforme */}
                        <View style={[styles.platformBadge, { backgroundColor: plat === 'tiktok' ? '#000000' : '#FF0000' }]}>
                      ...
                      <View style={styles.chatCardInfo}>
                        <Text style={[styles.chatCardTitle, { color: colors.textPrimary }]} numberOfLines={2}>
                          {summary.title}
                        </Text>
                        <Text style={[styles.chatCardMeta, { color: colors.textTertiary }]} numberOfLines={1}>
                          {summary.videoInfo?.channel || 'Vidéo'} · <Ionicons name="chatbubble-outline" size={10} color={colors.textTertiary} /> Chat IA
                        </Text>
                      </View>
