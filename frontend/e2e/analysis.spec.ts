...
test.describe('Video Analysis', () => {
  ...
  test('pasting YouTube URL enables analyze button', async ({ page }) => {
    ...
    const urlInput = page.locator('input[placeholder*="youtube" i], input[placeholder*="url" i], input[placeholder*="coller" i], input[placeholder*="lien" i], input[type="url"]').first();

    if (await urlInput.isVisible()) {
      await urlInput.fill('https://www.youtube.com/watch?v=dQw4w9WgXcQ');

      // The analyze button should be enabled
      const analyzeBtn = page.locator('button:has-text("Analyser"), button:has-text("Analyze"), button[type="submit"]').first();
  ...
  test('launching analysis shows loader then result', async ({ page }) => {
    ...
    if (await urlInput.isVisible()) {
      ...
      if (await analyzeBtn.isVisible()) {
        ...
        const loader = page.locator('[class*="spinner"], [class*="loader"], [class*="loading"], [role="progressbar"], text=/chargement|loading|analyse en cours/i');
        // Either loader appears or we go directly to results
        await expect(loader.or(page.locator('text=/summary|résumé|synthèse/i'))).toBeVisible({ timeout: 15000 });
      }
    }
  });
    ...
    if (await urlInput.isVisible()) {
      ...
      if (await analyzeBtn.isVisible()) {
        ...
        if (!isDisabled) {
          ...
          await expect(page.locator('text=/invalide|invalid|url|erreur|error/i')).toBeVisible({ timeout: 5000 });
        } else {
