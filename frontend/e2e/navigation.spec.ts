...
test.describe('Navigation & Route Protection', () => {
  ...
  test('authenticated user can navigate between pages', async ({ page }) => {
    ...
    await page.route('**/api/auth/quota**', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ credits: 150, monthly_limit: 150, plan: 'free' }),
      });
    });
    ...
    // Navigate to history
    const historyLink = page.locator('a[href*="history"], a[href*="historique"], button:has-text("Historique"), button:has-text("History")').first();
  ...
  test('unknown route shows 404 or redirects', async ({ page }) => {
    ...
    const is404 = await page.locator('text=/404|not found|page introuvable|page non trouvée/i').isVisible({ timeout: 5000 }).catch(() => false);
