...
test.describe("Analysis - URL Submission", () => {
  test("should navigate to dashboard", async ({ page, context }) => {
    ...
    await expect(
      page.getByRole("textbox", { name: /youtube|url|paste/i }),
    ).toBeVisible();
  });
  ...
  test("should clear input after successful analysis", async ({
    page,
    context,
  }) => {
    await authenticateUser(page, context);
...
test.describe("Analysis - Results Viewing", () => {
  ...
  test("should show synthesis content", async ({ page, context }) => {
    ...
    if (await synthesisTab.isVisible()) {
      ...
      // Should show content
      await expect(page.getByText(/summary|analysis|content/i)).toBeVisible({
        timeout: 10000,
      });
    }
  });
  ...
  test("should show reliability tab", async ({ page, context }) => {
    ...
    const reliabilityTab = page.getByRole("tab", {
      name: /reliability|fact.*check|sources/i,
    });
...
test.describe("Analysis - Error Handling", () => {
  test("should handle API errors gracefully", async ({ page, context }) => {
    ...
    await expect(page.getByText(/error|failed|connection/i)).toBeVisible({
      timeout: 10000,
    });
  });
