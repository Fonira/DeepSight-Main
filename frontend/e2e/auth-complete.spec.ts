...
test.describe('Auth - Register Flow', () => {
  ...
  test('should validate password match', async ({ page }) => {
    ...
    await expect(page.getByText(/password.*match|not.*match|mismatch/i)).toBeVisible();
  });
  ...
  test('should show error on duplicate email', async ({ page }) => {
    ...
    await expect(page.getByText(/already.*exists|already.*registered|duplicate/i)).toBeVisible({ timeout: 10000 });
  });
});
...
test.describe('Auth - Logout Flow', () => {
  test('should logout from authenticated session', async ({ page, context }) => {
    ...
    const userMenuButton = page.getByRole('button', { name: /profile|account|menu/i }).first();
...
test.describe('Auth - Protected Routes', () => {
  ...
  test('should redirect to login for invalid token', async ({ page, context }) => {
    await context.addCookies([
      {
        name: 'access_token',
        value: 'invalid-token',
    ...
    await expect(page).toHaveTitle(/login|sign in/i, { timeout: 10000 }).catch(() => {});
  });
});
