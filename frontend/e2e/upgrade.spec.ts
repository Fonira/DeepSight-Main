...
test.describe('Upgrade Page', () => {
  ...
  test('upgrade page loads without JS errors', async ({ page }) => {
    ...
    const criticalErrors = jsErrors.filter(
      (msg) => !msg.includes('posthog') && !msg.includes('sentry') && !msg.includes('analytics')
    );
  ...
  test('all 4 plans are displayed', async ({ page }) => {
    ...
    await expect(page.locator('text=/gratuit|free/i').first()).toBeVisible({ timeout: 10000 });

    // Count plan cards — look for pricing-related elements
    const planCards = page.locator('[class*="plan"], [class*="pricing"], [class*="card"]').filter({
  ...
  test('plans show correct prices', async ({ page }) => {
    ...
    const hasPricing = pageContent?.includes('2.99') || pageContent?.includes('2,99');
    const hasProPricing = pageContent?.includes('12.99') || pageContent?.includes('12,99');
  ...
  test('clicking "Choisir" triggers Stripe checkout', async ({ page }) => {
    ...
    if (await chooseBtn.isVisible({ timeout: 10000 }).catch(() => false)) {
      ...
      const [popup] = await Promise.all([
        ...
        page.waitForURL(/stripe|checkout|billing/, { timeout: 5000 }).catch(() => null),
        chooseBtn.click(),
      ]);
