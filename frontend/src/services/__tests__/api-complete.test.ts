...
import { mockApi, resetAllMocks, simulateNetworkError } from '../../__tests__/mocks/api-mocks';
...
describe('API Client - HTTP Requests', () => {
  ...
  it('should include auth header in all requests', async () => {
    ...
    const result = await mockApi.video.analyze('https://youtube.com/watch?v=test');
...
describe('API Modules - Auth', () => {
  it('should register user', async () => {
    const result = await mockApi.auth.register('new@test.com', 'password123', 'newuser');
...
describe('API Client - Resilience', () => {
  it('should handle temporary failures gracefully', async () => {
    ...
    // First attempt fails
    await expect(mockApi.auth.login('test@test.com', 'password'))
      .rejects.toThrow('Network error');
