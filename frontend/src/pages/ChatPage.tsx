/**
 * 💬 CHAT PAGE — Page de chat IA dédiée
 *
 * Accessible depuis la sidebar → /chat
 * Permet de choisir une analyse puis de chatter en plein écran.
 * Réutilise l'API /api/chat/ask et /api/chat/history/{summaryId}.
 */
...
const ChatPage: React.FC = () => {
  ...
  const texts = language === 'fr' ? {
    title: 'Chat IA',
    subtitle: 'Discutez avec l\'IA sur vos analyses',
    searchPlaceholder: 'Rechercher une analyse...',
    selectAnalysis: 'Sélectionnez une analyse pour commencer',
    selectAnalysisDesc: 'Choisissez une vidéo analysée pour poser des questions à l\'IA',
    ...
    title: 'AI Chat',
    subtitle: 'Chat with AI about your analyses',
    searchPlaceholder: 'Search an analysis...',
    selectAnalysis: 'Select an analysis to start',
    selectAnalysisDesc: 'Choose an analyzed video to ask the AI questions',
  ...
  useEffect(() => {
    ...
    const fetchHistory = async () => {
      try {
        ...
        // Pas critique, on commence avec un historique vide
        setMessages([]);
      } finally {
  ...
  return (
    <div className="min-h-screen flex flex-col lg:flex-row">
      ...
      <div className={`
        ...
        flex-col w-full lg:w-80 xl:w-96 border-r border-border-subtle bg-bg-primary
      `}>
        {/* Header */}
        <div className="p-4 border-b border-border-subtle">
          <div className="flex items-center gap-3 mb-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-text-primary">{texts.title}</h1>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 rounded-lg bg-bg-secondary border border-border-subtle text-text-primary placeholder-text-muted text-sm focus:outline-none focus:ring-2 focus:ring-accent-primary/30 focus:border-accent-primary/50 transition-all"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {filteredAnalyses.map((analysis) => (
            <button
              ...
              className={`
                w-full flex items-center gap-3 p-3 border-b border-border-subtle/50
                transition-all text-left hover:bg-bg-secondary/80
                ${selectedAnalysis?.id === analysis.id ? 'bg-accent-primary/5 border-l-2 border-l-accent-primary' : ''}
              `}
            >
              {/* Thumbnail */}
              <div className="w-14 h-10 rounded-md overflow-hidden bg-bg-tertiary flex-shrink-0">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-xs font-medium text-text-primary truncate">{analysis.video_title}</p>
      ...
      <div className="flex-1 flex flex-col min-h-0 bg-bg-primary">
        {!selectedAnalysis ? (
          ...
          <>
            {/* Chat header */}
            <div className="flex items-center gap-3 p-3 border-b border-border-subtle bg-bg-secondary/30">
              <button
                onClick={() => {
                  ...
                  <div className="flex flex-wrap justify-center gap-2">
                    {texts.suggestions.map((suggestion, i) => (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-full bg-bg-secondary border border-border-subtle text-xs text-text-secondary hover:text-accent-primary hover:border-accent-primary/30 transition-all"
                      >
                        {suggestion}
                      </button>
                    ))}
                ...
                >
                  ...
                  <div className={`
                    ...
                    ${msg.role === 'user'
                      ...
                      : 'bg-bg-secondary border border-border-subtle rounded-bl-md'
                    }
                  `}>
                    {/* Web search badge */}
                    {msg.web_search_used && (
