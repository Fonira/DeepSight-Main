...
> = (() => {
  ...
  for (const [id, info] of Object.entries(PLANS_INFO)) {
    ...
    const price =
      info.priceMonthly > 0
        ? ` (€${(info.priceMonthly / 100).toFixed(2)})`
        : "";
...
export const AdminPage: React.FC = () => {
  ...
  useEffect(() => {
    if (isUserAdmin) {
      setLoading(true);
      Promise.all([loadStats(), loadUsers(), loadLogs()]).finally(() =>
        setLoading(false),
      );
    }
  ...
  const handleDeleteUser = async (userId: number) => {
    ...
    try {
      ...
      showToast(
        language === "fr" ? "Utilisateur supprimé" : "User deleted",
        "success",
      );
      await loadUsers();
      await loadStats();
                  ...
                  <div className="space-y-6">
                    ...
                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                      ...
                      <div className="card p-5">
                        ...
                        <p className="text-2xl font-semibold font-semibold">
                          €{stats.revenue_estimate.toFixed(0)}
                        </p>
                        <p className="text-sm text-text-tertiary">
                          {language === "fr"
                            ? "Revenus (mois)"
                    ...
                    <div className="card p-4">
                      <div className="flex flex-wrap gap-4">
                        ...
                        <select
                          ...
                          onChange={(e) => {
                            setUsersPlanFilter(e.target.value);
                            setUsersPage(1);
                          }}
                      ...
                      <div className="overflow-x-auto">
                        <table className="w-full">
                          ...
                          <tbody className="divide-y divide-border-subtle">
                            {users.map((u) => {
                              const planConfig =
                                PLAN_CONFIG[u.plan] || PLAN_CONFIG.free;
                              return (
                                <tr
                                  key={u.id}
                                ...
                                >
                                  ...
                                  <td className="px-4 py-3">
                                    <span
                                      className={`inline-flex px-2 py-1 rounded-full text-xs font-medium ${planConfig.bg} ${planConfig.color}`}
                                    >
                                      {planConfig.label}
                                    </span>
                                  </td>
                  ...
                  <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div className="card p-5">
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div className="flex justify-between">
                          <span className="text-text-secondary">
                            {language === "fr" ? "Estimé/mois" : "Est./month"}
                          </span>
                          <span className="font-medium text-green-600">
                            €{stats.revenue_estimate.toFixed(2)}
                          </span>
                        </div>
                      </div>
                    </div>
      ...
      {showCreditsModal && selectedUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
          <div className="bg-bg-primary rounded-xl shadow-xl w-full max-w-md mx-4 p-6">
            <div className="flex items-center justify-between mb-6">
              <h3 className="font-semibold text-lg">
                {language === "fr" ? "Ajouter des crédits" : "Add Credits"}
              </h3>
              <button
            ...
            <div className="mb-4">
              ...
              <p className="font-medium">{selectedUser.email}</p>
              <p className="text-sm text-text-tertiary">
                {language === "fr" ? "Crédits actuels" : "Current credits"}:{" "}
