...
export const MyAccount: React.FC = () => {
    ...
    const fetchStatus = async () => {
      try {
        ...
        setApiKey((prev) => ({
          ...prev,
          error: tr(
            "Impossible de charger le statut API",
  ...
  const handleManageSubscription = async () => {
    ...
    try {
      ...
      showToast(
        tr("Erreur lors de l'accès au portail", "Error accessing portal"),
        "error",
      );
    } finally {
  ...
  const handleCancelSubscription = async () => {
    ...
    try {
      ...
      showToast(
        tr(
          "Abonnement annulé. Accès maintenu jusqu'à la fin de la période payée.",
          "Subscription cancelled. Access kept until paid period ends.",
        ),
        "success",
      );
    ...
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : tr("Erreur lors de la génération", "Generation failed");
  ...
  const handleRegenerateKey = async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : tr("Erreur lors de la régénération", "Regeneration failed");
      setApiKey((prev) => ({ ...prev, error: message, loading: false }));
    }
  };
  ...
  const handleRevokeKey = async () => {
    ...
    try {
      ...
      const message =
        err instanceof Error
          ? err.message
          : tr("Erreur lors de la révocation", "Revocation failed");
      setApiKey((prev) => ({ ...prev, error: message, loading: false }));
    }
  };
                ...
                {planLoading ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {[...Array(8)].map((_, i) => (
                      <div
                        key={i}
                        className="h-20 rounded-lg bg-bg-tertiary animate-pulse"
                      />
                    ))}
                  </div>
                ) : myPlan ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      return (
                        <div className="p-3 rounded-lg bg-bg-tertiary border border-border-subtle">
                          <div className="flex items-center gap-2 mb-2">
                            <div className="w-8 h-8 rounded-lg bg-accent-primary/20 text-accent-primary flex items-center justify-center">
                          ...
                          <p className="text-xs text-text-secondary mb-1.5">
                            {unlimited
                              ...
                              : `${used} / ${limit} ${tr("utilisées", "used")}`}
                          </p>
                          {!unlimited && limit > 0 && (
                            <div className="h-1.5 rounded-full bg-bg-hover overflow-hidden">
                              <div
                                className={`h-full rounded-full transition-all ${pct >= 90 ? "bg-red-500" : pct >= 70 ? "bg-amber-500" : "bg-accent-primary"}`}
                                style={{ width: `${pct}%` }}
                              />
                            </div>
                          )}
                    ...
                    {(() => {
                      ...
                      return (
                        <div
                          className={`p-3 rounded-lg border ${enabled ? "bg-bg-tertiary border-border-subtle" : "bg-bg-secondary/50 border-dashed border-border-subtle"}`}
                        >
                          <div className="flex items-center gap-2 mb-2">
                            <div
                              className={`w-8 h-8 rounded-lg flex items-center justify-center ${enabled ? "bg-emerald-500/20 text-emerald-400" : "bg-bg-tertiary text-text-tertiary"}`}
                            >
                              <GraduationCap className="w-4 h-4" />
                            </div>
                            <p
                              className={`text-sm font-medium ${enabled ? "text-text-primary" : "text-text-tertiary"}`}
                            >
                              Flashcards
                            </p>
                          </div>
                    ...
                    {(() => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2 mb-2">
                            <div
                              className={`w-8 h-8 rounded-lg flex items-center justify-center ${enabled ? "bg-violet-500/20 text-violet-400" : "bg-bg-tertiary text-text-tertiary"}`}
                            >
                    ...
                    {(() => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2 mb-2">
                            <div
                              className={`w-8 h-8 rounded-lg flex items-center justify-center ${enabled ? "bg-cyan-500/20 text-cyan-400" : "bg-bg-tertiary text-text-tertiary"}`}
                            >
                          ...
                          {enabled ? (
                            <p className="text-xs text-text-secondary">
                              {unlimited
                                ? `${used} — \u221e`
                                : `${used} / ${limit} ${tr("ce mois", "this month")}`}
                            </p>
                          ) : (
                            <Link
                              to="/upgrade"
                    ...
                    {(() => {
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2 mb-2">
                            <div
                              className={`w-8 h-8 rounded-lg flex items-center justify-center ${enabled ? "bg-amber-500/20 text-amber-400" : "bg-bg-tertiary text-text-tertiary"}`}
                            >
            ...
            <section className="card">
              ...
              <div className="panel-body space-y-4">
                ...
                <div
                  ...
                  style={
                    myPlan
                      ? { backgroundColor: `${myPlan.plan_color}15` }
                      : undefined
                  }
                >
                  <div className="flex items-center justify-between">
                    <div>
                      <p
                        ...
                        style={
                          myPlan ? { color: myPlan.plan_color } : undefined
                        }
                      >
                        {myPlan
                          ? `${myPlan.plan_icon} Plan ${myPlan.plan_name}`
                          : `${currentPlan.icon} Plan ${currentPlan.label}`}
                      </p>
                      <p className="text-sm text-text-tertiary mt-1">
                        {(() => {
                          const price =
                            planPrices[myPlan?.plan || user?.plan || "free"] ||
                            0;
                          return price > 0
                  ...
                  {myPlan?.subscription?.current_period_end && (
                    <p className="text-xs text-text-tertiary mt-3 flex items-center gap-1.5">
                      ...
                      {tr("Prochain renouvellement :", "Next renewal:")}{" "}
                      {formatDate(myPlan.subscription.current_period_end)}
                    </p>
                  )}
                </div>
            ...
            <section className="card">
              <div className="panel-header">
                <h2 className="font-semibold text-text-primary flex items-center gap-2">
                  ...
                  {isPaidUser && (
                    <span className="px-2 py-0.5 rounded-full text-xs font-medium bg-indigo-500/10 text-indigo-400">
                      Pro
                    </span>
                  )}
                </h2>
              ...
              <div className="panel-body">
                {!isPaidUser ? (
                  <div className="text-center py-6">
                    ...
                    <Link
                      to="/upgrade"
                      className="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-indigo-500/10 text-indigo-400 text-sm font-medium hover:bg-indigo-500/20 transition-colors"
                    >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex items-center justify-between p-4 rounded-lg bg-bg-tertiary">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-text-tertiary">
                            {apiKey.status?.has_api_key
                              ? tr(
                                  `Créée le ${formatDate(apiKey.status.created_at)}`,
                      ...
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-medium ${apiKey.status?.has_api_key ? "bg-success/10 text-success" : "bg-warning/10 text-warning"}`}
                      >
                        {apiKey.status?.has_api_key
                          ? tr("Active", "Active")
                      ...
                      <div className="p-4 rounded-lg bg-warning/10 border border-warning/30">
                        <p className="text-sm text-warning mb-3 flex items-center gap-2">
                          ...
                          {apiKey.confirmAction === "regenerate"
                            ...
                            : tr(
                                "Révoquer la clé désactivera l'accès API. Continuer ?",
                        ...
                        {!apiKey.status?.has_api_key ? (
                          ...
                          <>
                            <button
                              ...
                              disabled={apiKey.loading}
                              className="flex items-center gap-2 px-4 py-2 rounded-lg bg-bg-tertiary border border-border-default hover:bg-bg-hover text-sm disabled:opacity-50"
                            >
                            ...
                            <button
                              ...
                              disabled={apiKey.loading}
                              className="flex items-center gap-2 px-4 py-2 rounded-lg text-error hover:bg-error/10 text-sm disabled:opacity-50"
                            >
                ...
                <div className="flex items-center justify-between p-4 rounded-lg bg-bg-tertiary border border-border-subtle opacity-60">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-text-tertiary">
                        {tr(
                          "Connexion via Google - pas de mot de passe",
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-text-tertiary">
                        {tr(
                          "Se déconnecter de Deep Sight",
                          "Sign out of Deep Sight",
                        )}
                      </p>
                    </div>
                  </div>
                ...
                {!showDeleteConfirm ? (
                  ...
                  <div className="p-4 rounded-lg bg-error/10 border border-error/30">
                    <p className="text-sm text-error mb-4 flex items-start gap-2">
                      <AlertTriangle className="w-4 h-4 flex-shrink-0 mt-0.5" />
                      {tr(
                        "⚠️ Êtes-vous sûr ? Toutes vos données seront perdues.",
                        "⚠️ Are you sure? All your data will be permanently lost.",
                      )}
                    </p>
