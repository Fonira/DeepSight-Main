...
const EPISTEMIC_MARKERS: Record<string, {
  color: string;
  bg: string;
  borderColor: string;
  label: string;
  ...
  'SOLIDE': { color: '#22C55E', bg: 'rgba(34,197,94,0.10)', borderColor: 'rgba(34,197,94,0.25)', label: 'Établi', emoji: '✅' },
  'PLAUSIBLE': { color: '#3B82F6', bg: 'rgba(59,130,246,0.10)', borderColor: 'rgba(59,130,246,0.25)', label: 'Probable', emoji: '🔵' },
  'INCERTAIN': { color: '#F59E0B', bg: 'rgba(245,158,11,0.10)', borderColor: 'rgba(245,158,11,0.25)', label: 'Incertain', emoji: '🟡' },
  'A VERIFIER': { color: '#EF4444', bg: 'rgba(239,68,68,0.10)', borderColor: 'rgba(239,68,68,0.25)', label: 'À vérifier', emoji: '🔴' },
  'À VÉRIFIER': { color: '#EF4444', bg: 'rgba(239,68,68,0.10)', borderColor: 'rgba(239,68,68,0.25)', label: 'À vérifier', emoji: '🔴' },
};
...
function _cleanMarkers(text: string): string {
  if (!text) return '';
  return text.replace(/\[\[([^\]|]+?)(?:\|([^\]]+?))?\]\]/g, (_, term, display) => display || term);
}

function markTimecodes(text: string): string {
  if (!text) return '';
  return text.replace(/\[(\d{1,2}):(\d{2})(?::(\d{2}))?\]/g, (_match, p1, p2, p3) => {
    const part1 = parseInt(p1, 10);
    const part2 = parseInt(p2, 10);
    const part3 = p3 ? parseInt(p3, 10) : undefined;
    const seconds = part3 !== undefined ? part1 * 3600 + part2 * 60 + part3 : part1 * 60 + part2;
...
function markEpistemics(text: string): string {
  ...
  return text.replace(/\[(SOLIDE|PLAUSIBLE|INCERTAIN|A VERIFIER|À VÉRIFIER)\]/gi, (_match, marker) => {
...
function createComponents(onTimecodeClick?: (seconds: number) => void) {
        ...
        return (
          <button
            ...
            style={{
              ...
              padding: '2px 10px', margin: '0 3px', borderRadius: '6px',
              background: 'rgba(96, 165, 250, 0.12)', color: '#60a5fa',
              fontFamily: 'ui-monospace, monospace', fontSize: '13px', fontWeight: 600,
              border: '1px solid rgba(96, 165, 250, 0.25)',
              cursor: onTimecodeClick ? 'pointer' : 'default',
              transition: 'all 0.2s ease', verticalAlign: 'middle',
            }}
            onMouseEnter={(e) => { e.currentTarget.style.background = 'rgba(96, 165, 250, 0.25)'; e.currentTarget.style.transform = 'scale(1.05)'; }}
            onMouseLeave={(e) => { e.currentTarget.style.background = 'rgba(96, 165, 250, 0.12)'; e.currentTarget.style.transform = 'scale(1)'; }}
  ...
  return {
    p: ({ children, ...props }: any) => (
      <p {...props} style={{ margin: '0.75em 0', lineHeight: 1.75, color: '#e2e8f0' }}>
    ...
    h1: ({ children, ...props }: any) => {
      ...
      return (
        <h1 {...props} style={{
          fontSize: '1.5em', fontWeight: 700, margin: '1.2em 0 0.6em',
          color: '#f1f5f9', borderBottom: '1px solid rgba(255,255,255,0.08)',
          paddingBottom: '0.4em', lineHeight: 1.3,
        }}>
          {emoji}{'  '}{enrichChildren(children)}
        </h1>
      );
    },
    h2: ({ children, ...props }: any) => {
      ...
      return (
        <h2 {...props} style={{
          fontSize: '1.3em', fontWeight: 700, margin: '1em 0 0.5em',
    ...
    h3: ({ children, ...props }: any) => (
      <h3 {...props} style={{
        fontSize: '1.1em', fontWeight: 600, margin: '0.8em 0 0.4em',
        color: '#a5b4fc', lineHeight: 1.3,
      }}>
        {enrichChildren(children)}
      </h3>
    ),
    ...
    ul: ({ children, ...props }: any) => (
      <ul {...props} style={{ margin: '0.5em 0', paddingLeft: '1.5em', listStyleType: 'disc' }}>{children}</ul>
    ),
    ol: ({ children, ...props }: any) => (
      <ol {...props} style={{ margin: '0.5em 0', paddingLeft: '1.5em' }}>{children}</ol>
    ),
    li: ({ children, ...props }: any) => (
      <li {...props} style={{ margin: '0.3em 0', lineHeight: 1.7, color: '#e2e8f0' }}>
        {enrichChildren(children)}
      </li>
    ),
    ...
    code: ({ inline, children, ...props }: any) => {
      ...
      return (
        <code {...props} style={{ fontFamily: 'ui-monospace, "JetBrains Mono", monospace' }}>
          {children}
        </code>
      );
    },
    ...
    th: ({ children, ...props }: any) => (
      <th {...props} style={{
        padding: '0.5em 1em', background: 'rgba(139, 92, 246, 0.08)',
    ...
    td: ({ children, ...props }: any) => (
      <td {...props} style={{ padding: '0.5em 1em', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
        {enrichChildren(children)}
      </td>
    ),
    ...
    // HR
    hr: (props: any) => (
      <hr {...props} style={{ border: 'none', borderTop: '1px solid rgba(255,255,255,0.08)', margin: '1.5em 0' }} />
    ),
  };
}
