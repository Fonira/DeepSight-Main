...
export const PlanBadge: React.FC<PlanBadgeProps> = ({ collapsed }) => {
  ...
  // Re-fetch quand user change (upgrade/downgrade) ou page redevient visible
  useEffect(() => {
    fetchPlan();
  }, [fetchPlan, user?.id, user?.plan]);

  useEffect(() => {
    const onVisible = () => {
      if (document.visibilityState === "visible") fetchPlan();
    };
    const onUserUpdated = () => fetchPlan();
    document.addEventListener("visibilitychange", onVisible);
    window.addEventListener("user:updated", onUserUpdated);
    window.addEventListener("auth:success", onUserUpdated);
    return () => {
      document.removeEventListener("visibilitychange", onVisible);
      window.removeEventListener("user:updated", onUserUpdated);
      window.removeEventListener("auth:success", onUserUpdated);
    };
  }, [fetchPlan]);
  ...
  const limit = Number(
    limits?.monthly_analyses ?? limits?.monthlyAnalyses ?? 0,
  );
      ...
      <div className="px-2 py-1.5">
        ...
        >
          <div
            ...
            style={{
              backgroundColor: `${plan.plan_color}20`,
