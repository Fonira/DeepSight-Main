...
export const FlashcardsTab: React.FC<FlashcardsTabProps> = ({
  ...
  const [fcStats, setFcStats] = useState<FlashcardStats>({ known: 0, unknown: 0, total: 0, percentage: 0 });

  const handleFcProgress = useCallback((_current: number, _total: number, stats: FlashcardStats) => {
    setFcProgress({ current: stats.known + stats.unknown, total: stats.total });
    setFcStats(stats);
  }, []);
