 ...
 */
export const parseAskQuestions = (
  content: string,
): { beforeQuestions: string; questions: string[] } => {
 ...
 */
const FormatQuestionText: React.FC<{ text: string; isPlaylist: boolean }> = ({
  text,
  isPlaylist,
}) => {
  const regex = /\[\[([^\]|]+)(?:\|([^\]]+))?\]\]/g;
  ...
  return (
    <div className="mt-4 pt-3 border-t border-gray-600/30">
      <div className="flex items-center gap-2 mb-3">
        ...
        >
          <Sparkles
            className={`w-3.5 h-3.5 ${isPlaylist ? "text-purple-300" : "text-amber-300"}`}
          />
        </div>
        <span
          className={`text-xs font-semibold ${isPlaylist ? "text-purple-300" : "text-amber-300"}`}
        >
 ...
 */
export const MessageWithClickableQuestions: React.FC<
  ClickableQuestionsProps
> = ({ content, onQuestionClick, variant = "video", disabled = false }) => {
