...
const ActionButton = React.forwardRef<HTMLButtonElement, ActionButtonProps>(
  ({ icon, label, onClick, disabled = false, active = false, showChevron = false }, ref) => (
    <motion.button
      ref={ref}
      onClick={onClick}
      disabled={disabled}
    ...
    >
      ...
      {showChevron && (
        <ChevronDown className={`w-3.5 h-3.5 transition-transform hidden sm:block ${active ? 'rotate-180' : ''}`} />
      )}
    </motion.button>
  ),
);
...
const ExportOption: React.FC<ExportOptionProps> = ({ icon, label, description, onClick, loading }) => (
  <button
    onClick={onClick}
    disabled={loading}
...
export const AnalysisActionBar: React.FC<AnalysisActionBarProps> = ({
  ...
  const handleCopy = useCallback(async () => {
    ...
    try {
      ...
      showToast(language === 'fr' ? 'Impossible de copier' : 'Copy failed', 'error');
    }
    ...
    try {
      ...
      if (navigator.share) {
        await navigator.share({
          title: summary.video_title ? `DeepSight — ${summary.video_title}` : 'DeepSight Analysis',
          url: share_url,
        });
      } else {
        ...
        showToast(language === 'fr' ? 'Lien copié !' : 'Link copied!', 'success');
      }
    } catch (err: unknown) {
      if (err instanceof Error && err.name === 'AbortError') return;
      showToast(language === 'fr' ? 'Erreur de partage' : 'Share error', 'error');
    } finally {
      setSharing(false);
    }
  }, [summary?.video_id, summary?.video_title, sharing, language, showToast]);

  const handleExport = useCallback(async (format: 'pdf' | 'md' | 'txt') => {
    ...
    try {
      ...
      const nameMap: Record<string, string> = { pdf: 'PDF', md: 'Markdown', txt: 'Texte' };
      showToast(`Export ${nameMap[format]} ${language === 'fr' ? 'téléchargé' : 'downloaded'}`, 'success');
    } catch {
      showToast(language === 'fr' ? "Erreur d'export" : 'Export error', 'error');
    } finally {
  ...
  return (
    <>
      ...
      <div
        ...
        style={{
          background: 'linear-gradient(135deg, rgba(15,15,25,0.85) 0%, rgba(20,20,35,0.9) 100%)',
        ...
        <div className="px-3 py-3 sm:px-4 sm:py-3.5">
          <div className="flex flex-col lg:flex-row items-stretch lg:items-center gap-3">
            ...
            <motion.button
              ...
              `}
              style={!isVoiceLocked ? {
                background: 'linear-gradient(135deg, rgba(99,102,241,0.2) 0%, rgba(139,92,246,0.2) 50%, rgba(6,182,212,0.15) 100%)',
            ...
            >
              ...
              <div className="flex flex-col items-start min-w-0">
                ...
                `}>
                  {isVoiceLocked
                    ? (language === 'fr' ? 'Agent vocal verrouillé' : 'Voice Agent locked')
                    : (language === 'fr' ? 'Parler avec l\'Agent IA' : 'Talk to AI Agent')
                  }
                </span>
                <span className={`
                ...
                `}>
                  {isVoiceLocked
                    ? (language === 'fr' ? 'Disponible dès le plan Étudiant' : 'Available from Student plan')
                    : (language === 'fr' ? 'Propulsé par ElevenLabs' : 'Powered by ElevenLabs')
                  }
                </span>
              </div>
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap lg:flex-nowrap lg:ml-auto">
              ...
              <ActionButton
                ...
                label={copied ? (language === 'fr' ? 'Copié !' : 'Copied!') : (language === 'fr' ? 'Copier' : 'Copy')}
                onClick={handleCopy}
                active={copied}
              />
      ...
      {showExportMenu && createPortal(
        ...
        >
          <ExportOption
            ...
            label="PDF"
            description={language === 'fr' ? 'Export professionnel' : 'Professional layout'}
          ...
          <ExportOption
            ...
            label="Markdown"
            description={language === 'fr' ? 'Format éditable' : 'Editable format'}
          ...
          <ExportOption
            ...
            description={language === 'fr' ? 'Copier-coller universel' : 'Universal copy-paste'}
