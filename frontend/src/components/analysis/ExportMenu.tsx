...
export const ExportMenu: React.FC<ExportMenuProps> = ({
  ...
  const handleExport = useCallback(async (format: ExportFormat) => {
    if (format === 'audio' && !canExportAudio) {
      showToast('L\'export audio est disponible à partir du plan Pro', 'warning');
      return;
    }
    ...
    try {
      if (format === 'audio') {
        ...
        const blob = await videoApi.exportSummary(summaryId, formatMap[format]);
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        const extMap: Record<string, string> = { pdf: 'pdf', md: 'md', txt: 'txt' };
