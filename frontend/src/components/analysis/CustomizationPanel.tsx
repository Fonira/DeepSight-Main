...
export const CustomizationPanel: React.FC<CustomizationPanelProps> = ({
  ...
  const focusOptions = (
    Object.keys(ANALYSIS_FOCUS_CONFIG) as AnalysisFocus[]
  ).map((k) => ({
    key: k,
    ...ANALYSIS_FOCUS_CONFIG[k],
  }));
  const toneOptions = (Object.keys(WRITING_TONE_CONFIG) as WritingTone[]).map(
    (k) => ({
      key: k,
  ...
  const lengthOptions = (
    Object.keys(TARGET_LENGTH_CONFIG) as TargetLength[]
  ).map((k) => ({
    key: k,
    emoji:
      k === "short" ? "📄" : k === "medium" ? "📑" : k === "long" ? "📚" : "✨",
    label: TARGET_LENGTH_CONFIG[k].label,
    description: {
      fr: TARGET_LENGTH_CONFIG[k].wordRange.fr,
      en: TARGET_LENGTH_CONFIG[k].wordRange.en,
    },
  }));
    ...
    return (
      <div className="flex flex-wrap items-center gap-2">
        <span className="inline-flex items-center gap-1 px-3 py-1.5 bg-bg-tertiary text-text-tertiary rounded-full text-xs">
          {ANALYSIS_FOCUS_CONFIG[customization.analysisFocus].emoji}{" "}
          {ANALYSIS_FOCUS_CONFIG[customization.analysisFocus].label[language]}
        </span>
        <span className="inline-flex items-center gap-1 px-3 py-1.5 bg-bg-tertiary text-text-tertiary rounded-full text-xs">
          {WRITING_TONE_CONFIG[customization.writingTone].emoji}{" "}
          {WRITING_TONE_CONFIG[customization.writingTone].label[language]}
        </span>
      ...
      <div className="p-5 sm:p-6 space-y-5">
        ...
        <div className="space-y-2.5">
          ...
          <p className="text-[11px] text-white/25 pl-1">
            {
              ANALYSIS_FOCUS_CONFIG[customization.analysisFocus].description[
                language
              ]
            }
          </p>
        ...
        <div className="flex flex-col sm:flex-row sm:items-start gap-4">
          ...
          <div className="sm:w-48 space-y-2.5">
            ...
            <div className="relative">
              ...
              >
                {(Object.keys(OUTPUT_LANGUAGE_CONFIG) as OutputLanguage[]).map(
                  (lang) => {
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between px-4 py-3 rounded-xl bg-bg-tertiary border border-border-default">
                <div className="flex items-center gap-3">
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center ${customization.antiAIDetection ? "bg-emerald-500/15 text-emerald-400" : "bg-bg-hover text-text-muted"}`}
                  >
                    {customization.antiAIDetection ? (
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  <textarea
                    ...
                    onChange={(e) =>
                      updateCustomization({
                        userPrompt: e.target.value.slice(0, MAX_PROMPT_LENGTH),
                      })
                    }
                    disabled={disabled}
                    placeholder={t(
                  ...
                  {customization.userPrompt.length > 0 && (
                    <span
                      className={`absolute bottom-2 right-3 text-[10px] ${customization.userPrompt.length > MAX_PROMPT_LENGTH * 0.9 ? "text-orange-400" : "text-white/20"}`}
                    >
