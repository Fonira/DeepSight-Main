...
const TypewriterText = memo<TypewriterTextProps>(
  ({ text, isStreaming, maxHeight = "400px" }) => {
...
const ControlButtons = memo<ControlButtonsProps>(
  ({ status, onPause, onResume, onCancel, onRetry, lang }) => {
    const isActive = [
      "connecting",
      "metadata",
      "transcript",
  ...
  ({
    ...
    const statusMessage = useMemo(() => {
      const messages: Record<StreamStatus, Record<"fr" | "en", string>> = {
        ...
        metadata: {
          fr: "Récupération des métadonnées...",
    ...
    return (
      <div
        className={`bg-bg-elevated rounded-xl border border-border-default overflow-hidden ${className}`}
      >
        {/* Header */}
