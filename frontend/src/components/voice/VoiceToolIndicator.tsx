...
const TOOL_CONFIG: Record<string, { icon: React.ElementType; label: string }> = {
  ...
  search_in_transcript: { icon: FileSearch, label: 'Recherche dans le transcript...' },
  get_analysis_section: { icon: FileSearch, label: 'Récupération de l\'analyse...' },
...
export function VoiceToolIndicator({ toolName, isActive }: VoiceToolIndicatorProps) {
  ...
  return (
    <div className="mb-3 animate-fade-in">
      <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/10 backdrop-blur-sm border border-white/10">
        ...
        <span className="text-xs text-white/70 animate-pulse">{config.label}</span>
      </div>
    </div>
  );
}
