...
describe('VoiceWaveform', () => {
  ...
  it('affiche le bon aria-label pour mode idle', () => {
    ...
    expect(screen.getByRole('img').getAttribute('aria-label')).toBe('Audio idle');
  });

  it('affiche le bon aria-label pour mode user', () => {
    render(<VoiceWaveform mode="user" />);
    expect(screen.getByRole('img').getAttribute('aria-label')).toBe('User speaking');
  });

  it('affiche le bon aria-label pour mode ai', () => {
    render(<VoiceWaveform mode="ai" />);
    expect(screen.getByRole('img').getAttribute('aria-label')).toBe('AI speaking');
  });
  ...
  it('clamp l\'intensité entre 0 et 1', () => {
    ...
    const { unmount: unmount1 } = render(<VoiceWaveform mode="user" intensity={-0.5} />);
    unmount1();

    const { unmount: unmount2 } = render(<VoiceWaveform mode="user" intensity={2.0} />);
    unmount2();
    // No error = pass
  });
