...
const VoiceSettings: React.FC<VoiceSettingsProps> = ({ onClose, compact = false }) => {
  // ── State ──────────────────────────────────────────────────────────────
  ...
  // ── Save handler ───────────────────────────────────────────────────────
  const savePreferences = useCallback(async (updates: Partial<VoicePreferences>) => {
  ...
  if (!preferences || !catalog) {
    return (
      <div className="p-8 text-center">
        <p className="text-red-400">{error || 'Impossible de charger les préférences.'}</p>
      </div>
    );
  }
    ...
    <div className={`${compact ? '' : 'max-w-4xl mx-auto'} space-y-8`}>
          ...
          {filteredVoices.map((voice: VoiceCatalogEntry) => {
            ...
            return (
              <div
                ...
                `}
                onClick={() => savePreferences({ voice_id: voice.voice_id, voice_name: voice.name })}
              >
                {voice.recommended && (
                ...
                <div className="flex items-start gap-3">
                  <div className="flex-1">
                    ...
                    <p className="text-white/50 text-xs mt-1">{voice.description_fr}</p>
                    <div className="flex items-center gap-2 mt-2">
                      <span className="text-[10px] text-white/30">{voice.accent}</span>
                      <span className="text-white/20">·</span>
                      <span className="text-[10px] text-white/30">{voice.use_case}</span>
                    </div>
                  </div>
                  <button
                    onClick={(e) => {
      ...
      <section className="bg-white/5 border border-white/10 rounded-2xl p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {/* TTS Model */}
          <div>
            <p className="text-white/50 text-sm mb-2">Lecture TTS (résumés, synthèse)</p>
                ...
                >
                  <div className="flex items-center justify-between">
                    ...
                    <span className={`text-[10px] px-2 py-0.5 rounded-full ${
                      model.latency === 'lowest' ? 'bg-emerald-500/20 text-emerald-300' :
                      model.latency === 'low' ? 'bg-blue-500/20 text-blue-300' :
                      'bg-amber-500/20 text-amber-300'
                    }`}>
                      {model.latency === 'lowest' ? 'Ultra-rapide' : model.latency === 'low' ? 'Rapide' : 'Haute qualité'}
                    </span>
                  </div>
                  <p className="text-white/40 text-xs mt-1">{model.description_fr}</p>
                </button>
              ))}
            </div>
          </div>
          ...
          {/* Voice Chat Model */}
          <div>
            <p className="text-white/50 text-sm mb-2">Chat vocal (temps réel)</p>
            <div className="space-y-2">
              {catalog.models.map((model: VoiceModel) => (
                <button
                  key={`vc-${model.id}`}
                  onClick={() => savePreferences({ voice_chat_model: model.id })}
                  disabled={saving}
                  className={`
                    w-full text-left p-3 rounded-xl transition-all
