...
describe("CookieBanner", () => {
  ...
  it("should not render if consent already given", () => {
    ...
    render(<CookieBanner />);
    expect(
      screen.queryByText(/nous respectons votre vie privée/i),
  ...
  it("should show banner on first visit after delay", async () => {
    ...
    await waitFor(
      () => {
        expect(
          screen.getByText(/nous respectons votre vie privée/i),
        ).toBeInTheDocument();
      },
  ...
  it("should store analytics=false when refusing all", async () => {
    ...
    const refuseButton = await waitFor(() => screen.getByText("Tout refuser"), {
      timeout: 2000,
    });
