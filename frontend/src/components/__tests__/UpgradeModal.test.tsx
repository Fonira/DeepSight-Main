...
import { renderWithProviders, screen, userEvent, waitFor, act } from '../../__tests__/test-utils';
...
describe('UpgradeModal - Visibility', () => {
  it('should not render modal content by default', () => {
    ...
    expect(screen.queryByText(/Fonctionnalité verrouillée|Quota atteint|Vidéo trop longue/i)).not.toBeInTheDocument();
  });
  ...
  it('should show modal after dispatching show-upgrade-modal event', async () => {
    ...
    await showModal(
      { type: 'feature_locked', feature_label: 'Mind Map', required_plan_name: 'Pro', required_plan_price: 1299 },
      /Mind Map/
    );
...
describe('UpgradeModal - feature_locked', () => {
  it('should display feature_locked with correct content', async () => {
    ...
    await showModal(
      { type: 'feature_locked', feature_label: 'Playlist Analysis', required_plan_name: 'Pro', required_plan_price: 1299 },
      /Playlist Analysis/
    );
  ...
  it('should show upgrade button', async () => {
    ...
    await showModal(
      { type: 'feature_locked', feature_label: 'Web Search', required_plan_name: 'Starter', required_plan_price: 599 },
      /Web Search/
    );

    // FR: "Passer à Starter"
    const buttons = screen.getAllByRole('button');
    const upgradeButton = buttons.find(b => b.textContent?.includes('Passer à'));
...
describe('UpgradeModal - quota_exceeded', () => {
  ...
  it('should display default message when no custom message', async () => {
    ...
    expect(screen.getByText(/Vous avez atteint la limite/i)).toBeInTheDocument();
  });
  ...
  it('should show "Voir les plans" button', async () => {
    ...
    // FR: "Voir les plans"
    const buttons = screen.getAllByRole('button');
    const ctaButton = buttons.find(b => b.textContent?.includes('Voir les plans'));
...
describe('UpgradeModal - video_too_long', () => {
  it('should display video_too_long with duration info', async () => {
    ...
    await showModal(
      { type: 'video_too_long', video_duration_min: 180, max_duration_min: 120 },
      /Vidéo trop longue/
    );
  ...
  it('should show plan duration limits', async () => {
    ...
    await showModal(
      { type: 'video_too_long', video_duration_min: 300, max_duration_min: 240 },
      /Vidéo trop longue/
    );
  ...
  it('should use default message if durations not provided', async () => {
    ...
    expect(screen.getByText(/Cette vidéo dépasse la durée maximale/i)).toBeInTheDocument();
  });
});
...
describe('UpgradeModal - Close', () => {
  it('should close modal when X button clicked', async () => {
    ...
    await showModal(
      { type: 'feature_locked', feature_label: 'Test Feature', required_plan_name: 'Pro' },
      /Test Feature/
    );
  ...
  it('should close modal when backdrop clicked', async () => {
    ...
    await showModal(
      { type: 'feature_locked', feature_label: 'Test', required_plan_name: 'Pro' },
      /^Test$/
    );
...
describe('UpgradeModal - Navigation', () => {
  it('should navigate to /upgrade when upgrade button clicked', async () => {
    ...
    await showModal(
      { type: 'feature_locked', feature_label: 'Export PDF', required_plan_name: 'Pro', required_plan_price: 1299 },
      /Export PDF/
    );
...
describe('UpgradeModal - i18n', () => {
  it('should display text in French by default', async () => {
    ...
    await showModal(
      { type: 'feature_locked', feature_label: 'Mind Map', required_plan_name: 'Pro' },
      /Mind Map/
    );
  ...
  it('should show correct button labels in French', async () => {
    ...
    const buttons = screen.getAllByRole('button');
    expect(buttons.find(b => b.textContent?.includes('Voir les plans'))).toBeTruthy();
...
describe('UpgradeModal - Event Handling', () => {
  it('should update modal when new event dispatched while open', async () => {
    ...
    await showModal(
      { type: 'feature_locked', feature_label: 'Feature A', required_plan_name: 'Pro' },
      /Feature A/
    );
...
describe('UpgradeModal - Cleanup', () => {
  it('should cleanup event listener on unmount', () => {
    ...
    expect(removeEventListenerSpy).toHaveBeenCalledWith('show-upgrade-modal', expect.any(Function));
