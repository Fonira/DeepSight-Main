...
export const BadgeGrid: React.FC<BadgeGridProps> = ({ earned, locked }) => {
  ...
  const grouped = useMemo(() => {
    ...
    const all = [
      ...earned.map((b) => ({ badge: b as BadgeEarned | BadgeLocked, isEarned: true })),
      ...locked.map((b) => ({ badge: b as BadgeEarned | BadgeLocked, isEarned: false })),
    ];

    const groups: Record<Rarity, { badge: BadgeEarned | BadgeLocked; isEarned: boolean }[]> = {
      legendary: [],
      epic: [],
      rare: [],
      common: [],
        ...
        return (
          <div key={rarity}>
            ...
            >
              {items.map((item) => (
                <BadgeCard
                  key={'badge_id' in item.badge ? item.badge.badge_id : item.badge.name}
                  badge={item.badge}
