...
def analyze_freshness(
    video_date: str, video_title: str, video_description: str = "", transcript_excerpt: str = ""
) -> FreshnessResult:
    """
    ...
    if is_fast_changing and age_days > max_fresh_days:
        ...
        if warning_level == "critical":
            ...
            warning_message = (
                f"📅 Cette vidéo sur {detected_topic} a {age_days} jours. Certaines informations peuvent avoir évolué."
            )
            recommendations = [
        ...
        years = age_days // 365
        warning_message = (
            f"📅 Vidéo publiée il y a {years} an{'s' if years > 1 else ''}. Vérifiez l'actualité des informations."
        )
        recommendations = [
...
def analyze_claims_lite(text: str, video_title: str = "", lang: str = "fr") -> FactCheckLiteResult:
    """
    ...
    for claim_type, patterns in CLAIM_PATTERNS.items():
        for pattern in patterns:
            ...
            for match in matches:
                ...
                confidence, risk_level = _calculate_claim_risk(context, claim_type, full_text)
...
def _calculate_claim_risk(context: str, claim_type: str, full_text: str) -> Tuple[int, str]:
...
def _generate_risk_summary(
    confidence: int, high_risk: List[ClaimAnalysis], medium_risk: List[ClaimAnalysis], lang: str
) -> str:
...
def _generate_verification_suggestions(
    high_risk: List[ClaimAnalysis], medium_risk: List[ClaimAnalysis], lang: str
) -> List[str]:
    ...
    if has_stats:
        suggestions.append(
            ...
            if lang == "fr"
            else "Verify cited statistics on official sources like national statistics offices"
        )
    ...
    if has_vague:
        suggestions.append(
            ...
            if lang == "fr"
            else "Several sources are vague - look for original studies/reports"
        )
    ...
    if has_extraordinary:
        suggestions.append(
            ...
            if lang == "fr"
            else "Extraordinary claims require extraordinary evidence"
        )
    ...
    if not suggestions:
        suggestions.append(
            ...
            if lang == "fr"
            else "Cross-reference information with other sources for better reliability"
        )
...
def analyze_content_reliability(
    video_date: str, video_title: str, summary_content: str, video_description: str = "", lang: str = "fr"
) -> Dict[str, Any]:
    """
    ...
    # Fact-check LITE
    fact_check = analyze_claims_lite(text=summary_content, video_title=video_title, lang=lang)
