...
from .schemas import YouTubeComment, CommentsAnalysis, SentimentType, CommentCategory
...
async def fetch_youtube_comments(
    ...
    try:
        ...
        cmd = [
            ...
            "--no-write-info-json",
            "--extractor-args",
            f"youtube:comment_sort={sort_by};max_comments={limit}",
            "-o",
            temp_path.replace(".json", ""),
...
async def fetch_comments_via_api(video_id: str, api_key: str, limit: int = 100) -> List[Dict[str, Any]]:
    """
        ...
        while len(comments) < limit:
            ...
            try:
                response = await client.get(
                    "https://www.googleapis.com/youtube/v3/commentThreads", params=params, timeout=30.0
                )
...
def categorize_comment(text: str, sentiment: SentimentType, is_constructive: bool, is_toxic: bool) -> CommentCategory:
    """
...
def extract_questions(text: str) -> List[str]:
    ...
    questions = [
        s.strip() + "?"
        for s in sentences
        if "?" in s or re.search(r"^(pourquoi|comment|quand|où|qui|what|why|how|when|where|who)", s.strip().lower())
    ]
...
async def analyze_comments_with_ai(
    comments: List[Dict[str, Any]], video_title: str, lang: str = "fr", model: str = "mistral-small-2603"
) -> Dict[str, Any]:
    """
    ...
    comments_text = "\n".join([f"[{c.get('like_count', 0)}👍] {c.get('text', '')[:300]}" for c in top_comments])
...
async def analyze_comments(
    ...
    if not raw_comments:
        ...
        return CommentsAnalysis(video_id=video_id, total_comments=0, analyzed_count=0)
    ...
    top_constructive = sorted(
        [c for c in analyzed_comments if c.is_constructive], key=lambda x: x.like_count, reverse=True
    )[:5]
    ...
    if use_ai and n_analyzed >= 10:
        ai_analysis = await analyze_comments_with_ai(
            [{"text": c.text, "like_count": c.like_count} for c in analyzed_comments], video_title, lang, model
        )
    ...
    print(
        f"✅ [COMMENTS] Analysis complete: {n_analyzed} comments, "
        f"sentiment={avg_sentiment:.2f}, constructive={constructive_ratio:.1%}",
        flush=True,
    )
