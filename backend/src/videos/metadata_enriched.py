...
PROPAGANDA_TECHNIQUES = {
    ...
    "strawman": [r"(?:ils?|they)\s+(?:pr[ée]tendent|claim)\s+que", r"(?:selon\s+eux|according\s+to\s+them)"],
}
...
def detect_sponsorship(title: str, description: str, transcript: str = "") -> SponsorshipInfo:
    """
    ...
    return SponsorshipInfo(
        type=detected_type, brands=brands_found[:10], disclosed=disclosed, confidence=confidence, evidence=evidence[:5]
    )
...
def detect_propaganda_risk(title: str, description: str, transcript: str = "", channel: str = "") -> PropagandaAnalysis:
    """
        ...
        for pattern in patterns:
            ...
            for match in matches:
                ...
                problematic_segments.append(
                    {"technique": technique, "match": match.group(0), "context": f"...{context}..."}
                )
...
async def extract_public_figures(
    title: str, description: str, transcript: str = "", lang: str = "fr", use_ai: bool = True
) -> List[PublicFigure]:
    """
    ...
    if not use_ai:
        ...
        for name, count in sorted(name_counts.items(), key=lambda x: -x[1])[:10]:
            figures.append(PublicFigure(name=name, mentions_count=count))
...
def analyze_publication_intent(
    title: str, description: str, transcript: str = "", tags: List[str] = None, category: str = ""
) -> PublicationIntent:
    """
    ...
    # Calculer les scores
    scores = {"educational": 0.0, "entertainment": 0.0, "commercial": 0.0, "persuasion": 0.0}
...
async def get_enriched_metadata(
    ...
    print(
        ...
        f"propaganda={'analyzed' if propaganda else 'skipped'}",
        flush=True,
    )
