...
def _chunk_by_timestamps(segments: list[tuple[int, str]], chunk_duration: int, video_duration: int) -> List[ChunkData]:
    """
    ...
    for seconds, text in segments:
        if current_texts and (seconds - chunk_start) >= chunk_duration:
            ...
            chunks.append(
                ChunkData(
                    index=chunk_index, start_seconds=chunk_start, end_seconds=seconds, text=" ".join(current_texts)
                )
            )
            chunk_index += 1
            current_texts = []
    ...
    if current_texts:
        chunks.append(
            ChunkData(
                index=chunk_index, start_seconds=chunk_start, end_seconds=video_duration, text=" ".join(current_texts)
            )
        )
...
def _chunk_by_characters(transcript: str, chunk_duration: int, video_duration: int) -> List[ChunkData]:
    """
...
def chunk_transcript(transcript: str, video_duration_seconds: int) -> List[ChunkData]:
    """
    ...
    if chunk_duration == 0:
        ...
        return [ChunkData(index=0, start_seconds=0, end_seconds=video_duration_seconds, text=transcript.strip())]
...
async def digest_chunk(chunk: ChunkData, video_title: str, category: str = "general") -> str:
    """
...
[{_format_time(chunk.start_seconds)}] L'auteur explique que... Puis vers [{_format_time((chunk.start_seconds + chunk.end_seconds) // 2)}], il aborde..."""
    ...
    try:
        ...
        logger.error("Failed to digest chunk", extra={"chunk_index": chunk.index, "error": str(e)})
...
async def build_full_digest(
    chunks: List[ChunkData], video_title: str, video_duration: int, category: str = "general"
) -> str:
    """
...
"""
    ...
    try:
        ...
        logger.info("Full digest built", extra={"chunk_count": len(chunks), "digest_chars": len(full_digest)})
        return full_digest
    except Exception as e:
        logger.error("Failed to build full digest", extra={"chunk_count": len(chunks), "error": str(e)})
...
async def process_video_chunks(
    transcript: str, video_duration: int, video_title: str, summary_id: int, db: AsyncSession, category: str = "general"
) -> str:
    """
    ...
    logger.info(
        ...
        extra={"summary_id": summary_id, "transcript_chars": len(transcript), "duration_seconds": video_duration},
    )
    ...
    logger.info("All chunks digested", extra={"summary_id": summary_id, "chunk_count": len(chunks)})
    ...
    logger.info("Chunks stored in database", extra={"summary_id": summary_id, "chunk_count": len(chunks)})
    ...
    logger.info(
        ...
        extra={"summary_id": summary_id, "chunk_count": len(chunks), "full_digest_chars": len(full_digest)},
    )
...
async def build_full_digest_from_existing_chunks(
    ...
    if not db_chunks:
        logger.info("no_existing_chunks_for_digest", extra={"summary_id": summary_id})
        return ""

    logger.info("building_digest_from_existing_chunks", extra={"summary_id": summary_id, "chunk_count": len(db_chunks)})
    ...
    full_digest = await build_full_digest(chunks_data, video_title, video_duration, category)
