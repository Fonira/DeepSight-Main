...
async def ask_question_v4(
    request: ChatRequest, current_user: User = Depends(get_current_user), session: AsyncSession = Depends(get_session)
):
    """
    ...
    if V4_AVAILABLE:
        ...
        if "error" in result:
            raise HTTPException(status_code=429 if "limit" in result["error"] else 404, detail=result["error"])
...
async def _ask_question_legacy(request: ChatRequest, current_user: User, session: AsyncSession) -> ChatResponseV4:
    ...
    if not can_ask:
        raise HTTPException(
            status_code=429, detail=f"Chat limit reached: {reason}", headers={"X-Quota-Info": str(quota_info)}
        )
    ...
    try:
        ...
        should_search, trigger_reason = needs_web_search_for_chat(request.question, summary.video_title)
    except ImportError:
        should_search = False
    ...
    if use_perplexity:
        ...
        if can_search:
            ...
            web_search_result = await search_with_perplexity(
                request.question, f"{summary.video_title}: {_context_preview}", summary.lang or "fr"
            )
    ...
    assistant_msg_id = await save_chat_message(session, current_user.id, request.summary_id, "assistant", response)
...
async def ask_question_stream(
    ...
    return StreamingResponse(
        generate(), media_type="text/event-stream", headers={"Cache-Control": "no-cache", "Connection": "keep-alive"}
    )
...
async def get_history(
    ...
    return ChatHistoryResponse(messages=[ChatMessage(**m) for m in messages], quota_info=quota_info)
...
async def get_quota_info_by_path(
    ...
    """
    can_ask, reason, quota_info = await check_chat_quota(session, current_user.id, summary_id)

    can_search, web_used, web_limit = await check_web_search_quota(session, current_user.id)
...
async def get_enrichment_info(current_user: User = Depends(get_current_user)):
    """
...
async def ask_question_by_path(
    ...
    # Fallback legacy
    request = ChatRequest(question=question, summary_id=summary_id, use_web_search=use_web_search, mode=mode)
    return await _ask_question_legacy(request, current_user, session)
