...
class CacheService:
    ...
    async def get_or_set(self, key: str, factory: Callable[[], T], ttl: Optional[int] = None) -> Optional[T]:
        """
...
class TranscriptCacheMetrics:
    ...
    async def increment(self, key: str, amount: int = 1) -> None:
        ...
        try:
            if cache_service.is_redis:
                await cache_service.backend.redis.incrby(f"deepsight:{redis_key}", amount)
            else:
    ...
    async def get_all(self) -> dict:
        ...
        for name, redis_key in self.KEYS.items():
            try:
                if cache_service.is_redis:
                    val = await cache_service.backend.redis.get(f"deepsight:{redis_key}")
        ...
        metrics["hit_rate_percent"] = round((total_hits / total_lookups * 100) if total_lookups > 0 else 0, 1)
        metrics["supadata_calls_avoided"] = total_hits
        metrics["estimated_cost_saved_usd"] = round(total_hits * self.SUPADATA_COST_PER_CALL, 2)
        metrics["estimated_cost_spent_usd"] = round(metrics["supadata_calls"] * self.SUPADATA_COST_PER_CALL, 2)
