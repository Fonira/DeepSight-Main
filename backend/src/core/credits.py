...
MODEL_COSTS = {
    ...
    "mistral-small-latest": {"analysis": 50, "chat": 5, "name": "Mistral Small (legacy)", "multiplier": 1.0},
    "mistral-medium-latest": {"analysis": 100, "chat": 10, "name": "Mistral Medium (legacy)", "multiplier": 2.0},
    "mistral-large-latest": {"analysis": 150, "chat": 15, "name": "Mistral Large (legacy)", "multiplier": 3.0},
}
...
def calculate_playlist_cost(
    video_count: int, model: str = "mistral-small-latest", with_web_search: bool = False
) -> Dict[str, Any]:
    """
...
async def check_credits(session: AsyncSession, user_id: int, required: int) -> Tuple[bool, int, str]:
    """
...
async def add_credits(
    session: AsyncSession, user_id: int, amount: int, reason: str, transaction_type: str = "purchase"
) -> Tuple[bool, int]:
    """
    ...
    print(
        f"💰 Credits: User {user_id} received {amount} credits ({transaction_type}). Balance: {user.credits}",
        flush=True,
    )
...
def get_cost_preview(action: str, model: str = "mistral-small-latest", **kwargs) -> Dict[str, Any]:
    """
    ...
    if action == "analysis":
        ...
        return calculate_playlist_cost(
            video_count=kwargs.get("video_count", 1), model=model, with_web_search=kwargs.get("with_web_search", False)
        )
    else:
...
def get_credits_summary() -> Dict[str, Any]:
    ...
    """
    return {
        "plans": {plan: {"monthly_credits": credits} for plan, credits in PLAN_CREDITS.items() if plan != "unlimited"},
        "models": {
            model_id: {
                "name": info["name"],
