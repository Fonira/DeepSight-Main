...
PLAN_HIERARCHY = {"free": 0, "starter": 1, "pro": 2, "expert": 3, "unlimited": 4}
...
async def update_profile(
    ...
    print(
        f"✅ Profile updated: {current_user.username} - lang={current_user.default_lang}, mode={current_user.default_mode}, model={current_user.default_model}",
        flush=True,
    )
...
async def upload_avatar(
    ...
    if file.content_type not in allowed_types:
        raise HTTPException(status_code=400, detail="Type de fichier non supporté. Acceptés: PNG, JPG, GIF, WebP")
    ...
    return {"success": True, "avatar_url": avatar_url, "message": "Avatar mis à jour avec succès"}


@router.delete("/avatar")
async def delete_avatar(current_user: User = Depends(get_current_user), session: AsyncSession = Depends(get_session)):
...
async def get_available_models(current_user: User = Depends(get_current_user)):
    ...
    return ModelsResponse(
        models=models, current_model=current_user.default_model or "mistral-small-2603", plan=user_plan
    )
