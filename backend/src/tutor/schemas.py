...
class SessionStartRequest(BaseModel):
    ...
    summary_id: Optional[int] = Field(
        None,
        description="ID de l'analyse vidéo source si concept vient de l'historique",
    )
...
class SessionStartResponse(BaseModel):
    ...
    audio_url: Optional[str] = Field(
        None,
        description="Présent si mode='voice' (V1.1 — None en V1.0 text-only)",
    )
...
class SessionTurnRequest(BaseModel):
    ...
    V1.0 : seul user_input (texte) est supporté côté serveur.
    audio_blob_b64 réservé pour V1.1 (Voxtral STT).
    """

    user_input: Optional[str] = Field(None, max_length=2000)
    audio_blob_b64: Optional[str] = Field(
        None,
        description="Audio user encodé base64 (V1.1 — non supporté V1.0)",
    )
