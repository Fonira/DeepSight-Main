...
async def get_tournesol_data(video_id: str):
    ...
    if not clean_id or len(clean_id) != 11:
        return TournesolResponse(found=False, error="Invalid video ID format")
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            response = await client.get(
                url, headers={"Accept": "application/json", "User-Agent": "DeepSight/1.0 (tournesol-integration)"}
            )

            logger.debug(f"Tournesol API response: {response.status_code} for {clean_id}")

            if response.status_code == 404:
                return TournesolResponse(found=False, error=None)

            if response.status_code != 200:
                return TournesolResponse(found=False, error=f"API returned {response.status_code}")
            ...
            logger.info(
                f"Tournesol data for {clean_id}: score={tournesol_score}, comparisons={n_comparisons}, contributors={n_contributors}"
            )
            ...
            entity = TournesolEntity(
                ...
                criteria_scores=[
                    CriteriaScore(criteria=c.get("criteria", ""), score=c.get("score")) for c in (criteria_scores or [])
                ]
                if criteria_scores
                else None,
            )

            return TournesolResponse(found=True, data=entity)
...
async def search_tournesol(request: SearchRequest):
    ...
    params = {"search": request.query, "limit": limit, "unsafe": "false"}
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            response = await client.get(
                ...
                params=params,
                headers={"Accept": "application/json", "User-Agent": "DeepSight/1.0 (tournesol-integration)"},
            )
            ...
            return SearchResponse(results=results[:limit], total=len(results), query=request.query)
...
async def get_recommendations(limit: int = 20, language: Optional[str] = None, date_gte: Optional[str] = None):
    """
    ...
    try:
        async with httpx.AsyncClient(timeout=15.0) as client:
            ...
            return SearchResponse(results=results, total=len(results), query="recommendations")
...
async def get_tournesol_batch(video_ids: str):
    ...
    return {"results": {vid: resp.dict() for vid, resp in results}}
