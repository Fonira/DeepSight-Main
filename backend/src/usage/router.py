...
async def get_usage_stats(current_user: User = Depends(get_current_user), session: AsyncSession = Depends(get_session)):
    """
    ...
    analyses_this_month_result = await session.execute(
        select(func.count(Summary.id)).where(
            and_(Summary.user_id == current_user.id, Summary.created_at >= month_start)
        )
    )
...
async def get_recent_transactions(
    current_user: User = Depends(get_current_user), session: AsyncSession = Depends(get_session), limit: int = 20
):
    """
...
async def get_plan_features(current_user: User = Depends(get_current_user)):
    """
    ...
    return {"current_plan": current_plan, "plans": plans_comparison}
...
async def get_detailed_usage(
    current_user: User = Depends(get_current_user), session: AsyncSession = Depends(get_session), days: int = 30
):
    """
    ...
    analyses_by_day_result = await session.execute(
        select(func.date(Summary.created_at).label("date"), func.count(Summary.id).label("count"))
    ...
    analyses_by_day = [{"date": str(row.date), "count": row.count} for row in analyses_by_day_result]

    # Crédits utilisés par type
    credits_by_type_result = await session.execute(
        select(CreditTransaction.transaction_type, func.sum(func.abs(CreditTransaction.amount)).label("total"))
        .where(CreditTransaction.user_id == current_user.id)
        .where(CreditTransaction.amount < 0)
        .where(CreditTransaction.created_at >= start_date)
        .group_by(CreditTransaction.transaction_type)
    )
    credits_by_type = {row.transaction_type or "other": int(row.total or 0) for row in credits_by_type_result}

    # Catégories analysées
    categories_result = await session.execute(
        select(Summary.category, func.count(Summary.id).label("count"))
        .where(Summary.user_id == current_user.id)
        .where(Summary.created_at >= start_date)
        .group_by(Summary.category)
        .order_by(func.count(Summary.id).desc())
    )
    categories = {row.category or "Autre": row.count for row in categories_result}

    # Modèles utilisés
    models_result = await session.execute(
        select(Summary.model_used, func.count(Summary.id).label("count"))
        .where(Summary.user_id == current_user.id)
        .where(Summary.created_at >= start_date)
        .where(Summary.model_used.isnot(None))
        .group_by(Summary.model_used)
    )
    models_used = {row.model_used: row.count for row in models_result}
...
async def estimate_cost(
    ...
    if action == "analysis":
        ...
        cost_info = calculate_chat_cost(model=model, with_web_search=with_web_search)
    elif action == "playlist":
        cost_info = calculate_playlist_cost(video_count=video_count, model=model, with_web_search=with_web_search)
    else:
