...
**Date** : 2026-04-26 (initial), addendum 2026-04-27 (validation utilisateur + palette logo officiel)
**Status** : Brainstormed + addendum validé, awaiting implementation plans (PR1/PR2/PR3/PR4)
**Owner** : Fonira
...
## 14. Addendum 2026-04-27 — Validation utilisateur + ajustements

### 14.1 Décisions actées (session brainstorming 2026-04-27)

| #   | Décision                                                             | Statut                |
| --- | -------------------------------------------------------------------- | --------------------- |
| 1   | Refonte complète (pas MVP additif sur v2)                            | ✅ confirmé           |
| 2   | Tournesol photoréaliste 3D (pas SVG plat)                            | ✅ confirmé           |
| 3   | Pipeline auto Three.js (`gen-sunflower-frames.mjs`)                  | ✅ confirmé           |
| 4   | 3 plateformes en scope (web + mobile + extension)                    | ✅ confirmé           |
| 5   | Mobile consomme `@deepsight/lighting-engine` (Metro fix obligatoire) | ✅ confirmé           |
| 6   | Modèle 3D fidèle à la palette logo officiel Tournesol                | ✅ NOUVEAU (cf. 14.2) |

### 14.2 Ajustement palette modèle 3D — fidélité au logo Tournesol officiel

La spec initiale (§5) prévoyait une palette tournesol "nature" (jaune `#fef9c3` → orange `#fbbf24` → marron `#b45309`). L'utilisateur précise que le modèle 3D doit **s'inspirer du logo officiel de l'API Tournesol** déjà utilisé dans le produit (`tournesol-icon.svg`, palette double couronne verte+jaune).

**Référence visuelle** : `frontend/public/platforms/tournesol-icon.svg` (43×40, déjà committé)

#### Palette à utiliser dans `gen-sunflower-frames.mjs` (variant jour)

| Élément                       | Couleur                          | Note                                    |
| ----------------------------- | -------------------------------- | --------------------------------------- |
| Pétales — couronne ext.       | `#9DC209` (vert lime Tournesol)  | Signature visuelle de la marque         |
| Pétales — couronne int.       | `#F3BE00` (jaune doré Tournesol) | Couronne intérieure du logo             |
| Pétales — gradient transition | mix linéaire vert→jaune          | Smooth shading 3D entre les 2 couronnes |
| Centre — disque externe       | `#BF5F06` (rouille Tournesol)    | Au lieu de `#a16207` marron             |
| Centre — disque interne       | `#7A3A03` → `#3D1D01`            | Profondeur graines                      |
| Graines (24-28 visibles)      | spirale Fibonacci, alpha 0.6→1   | Pattern fidèle au logo SVG              |
| Tige                          | absente du sprite                | (économie pixels, comme spec initiale)  |

#### Palette variant nuit (luminescent / bioluminescence)

Inchangée vs spec §5 : pétales émissifs cyan/violet `#4f46e5`, glow renderpass. **Mais** la silhouette des pétales doit conserver la signature double couronne du logo officiel (forme externe verte + interne dorée vues sous lumière émissive bleue).

#### Lighting offline

- Variant jour : HDRI golden hour + key light blanc-or 5500K → renforce la palette jaune/vert Tournesol
- Variant nuit : MeshStandardMaterial.emissiveIntensity = 0.8 sur les pétales (vert→émissif cyan), key light cool 6500K très basse intensity (0.15)

### 14.3 Roadmap d'exécution validée

```
Cette session — 4 plans détaillés à produire en parallèle

PR1 — feat/lighting-engine-v3-foundation (~2 j)
   Engine v3 (48 keyframes, frameIndex, nightMode, accents brand, daily seed,
   reading-zone cap, prefers-reduced-motion) + script gen-sunflower-frames.mjs
   avec palette logo officiel Tournesol + sprites WebP committed dans 3 dest. +
   tokens shift 3 plateformes + backend champ User.preferences.ambient_lighting_enabled
   + Vitest engine + tokens.

PR2 — feat/ambient-lighting-v3-web (~1.5 j) — parallèle PR3, PR4
   AmbientLightLayer rewrite (frontend/), SunflowerLayer route-aware (hero/mascot),
   AmbientLightingContext, useAmbientPreset rewrite (lighting-engine v3),
   vite-plugin-ambient-critical-css, Settings toggle, tests RTL + Playwright.

PR3 — feat/ambient-lighting-v3-mobile (~1.5 j) — parallèle PR2, PR4
   Metro config étendu (watchFolders + extraNodeModules pour résoudre
   @deepsight/lighting-engine), suppression useTimeOfDay legacy mobile,
   AmbientLightLayer rewrite, SunflowerLayer mascot-only, AmbientLightingContext,
   splash screen Expo avec PNG fallback du beam initial, AppState pause cross-fade,
   profile.tsx toggle, tests Jest+RNTL + Maestro snapshots.

PR4 — feat/ambient-lighting-v3-extension (~1.5 j) — parallèle PR2, PR3
   Sortie d'orphelinat des 2 AmbientLightLayer.tsx (sidepanel + viewer), création
   popup minimal sans SunflowerLayer (bundle <50KB), 3 entries (sidepanel, viewer,
   popup) avec leur Provider, copy-webpack-plugin pour sprites, html-webpack-plugin
   critical CSS inline, coordination BeamCard via Context (PR0 dépendance), tests Jest.

Hors scope cette session :
PR0 — déjà en cours sur worktree feat/extension-sidepanel-v3 (livre <BeamCard>)
PR5 — cleanup final (suppression v2 legacy, mise à jour PRD + CHANGELOG)
```

### 14.4 Conventions de plan

Chaque plan détaillé doit contenir :

1. **Liste exhaustive des fichiers** créés / modifiés / supprimés (paths absolus depuis racine du repo)
2. **Découpage en commits atomiques** (1 commit par changement logique cohérent)
3. **Dépendances externes** (npm packages à ajouter, versions, raison)
4. **Tests à écrire** (avec noms et critères de réussite)
5. **Critères de done** (commandes locales à passer vert : `npm run typecheck`, `npm run test`, `npm run lint`, `npm run build`)
6. **Ordre d'exécution** (top-down, indépendances marquées)
7. **Risques et mitigations** spécifiques à la PR
8. **Coordination cross-PR** (e.g., PR2 dépend de PR1 mergée pour avoir l'engine v3)

### 14.5 Hors scope explicitement reporté

- **PR0 (sidepanel-v3)** : continue sur worktree dédié, le hook `useBeamCardPropsFromPreset()` documenté ici sera ajouté quand PR0 livre `<BeamCard>` — n'est pas un blocker pour PR4
- **A11y avancée** (color contrast dynamique selon angle) : PR follow-up, hors scope v3 initial
- **Customization user de la palette** : rejeté (cf. §12)
- **Tournesol interactif / easter eggs** : reporté à v3.1
- **Refactor complet du theme system mobile** vers tokens design-tokens.css : hors scope (audit ciblé seulement)

### 14.6 Corrections de la spec après écriture des plans détaillés

Pendant l'écriture des 4 plans (4 agents Opus 4.7 dispatchés en parallèle 2026-04-27), trois écarts entre la spec initiale et l'état réel du codebase ont été détectés. La spec est rectifiée ici :

#### 14.6.1 Backend `User.preferences` n'existe pas encore

**Spec §8 affirmait** : « Le model `User` a déjà un champ `preferences` (JSON column) → on ajoute la clé sans migration breaking ».

**Réalité du codebase** (vérifiée par l'agent PR1 dans `backend/src/db/database.py`) : le model `User` n'a actuellement que des colonnes scalaires. Il n'y a pas de colonne `preferences` JSON.

**Correction** : PR1 inclut une **migration alembic 008** qui ajoute la colonne `preferences JSON DEFAULT '{}'` à la table `users`. L'helper `core/preferences.py` lit cette colonne avec `AMBIENT_LIGHTING_DEFAULT = True` en fallback si la clé n'existe pas. L'endpoint `PUT /api/auth/preferences` (à créer ou étendre) accepte `{ ambient_lighting_enabled: bool }`. L'endpoint `/api/auth/me` expose `preferences` dans la réponse.

#### 14.6.2 Popup Chrome retiré du manifest par PR0

**Spec §6.3 prévoyait** : 3 entry points (sidepanel + viewer + popup) avec `extension/src/popup/components/AmbientLightLayer.tsx` minimal.

**Réalité du codebase** (vérifiée par l'agent PR4) : la PR0 sidepanel-v3 a **supprimé `default_popup` du manifest**. Les 3 manifests (Chrome, Firefox, Safari) pointent désormais tous vers `sidepanel.html`. Le composant racine du popup a disparu du build.

**Correction** : PR4 scope effectif = **sidepanel + viewer uniquement**. La sous-section §6.3 listant le popup AmbientLightLayer minimal est obsolète. Si un popup minimal est jugé nécessaire ultérieurement (ex: rebrand "click-action"), ce sera une PR4-bis dédiée hors de cette refonte v3.

#### 14.6.3 Risque pipeline `gl` sur Windows

**Spec §5 prévoyait** : `Three.js headless via gl (canvas WebGL Node.js)`.

**Réalité du codebase / du tooling** : la lib npm `gl` (binding Mesa/OSMesa) **a un historique de fail au compile sur Windows native** (Visual Studio Build Tools, headers Mesa, etc.). Le risque est documenté mais non testé.

**Correction** : PR1 plan documente un **fallback `puppeteer`** (Chromium headless avec WebGL accéléré software). Stratégie d'exécution Task 8 :

1. Tenter `npm install gl` d'abord. Si échec sur Windows → bascule immédiatement sur `puppeteer` (~150 MB en deps additionnelles, mais marche cross-platform).
2. Le sprite généré doit être identique pixel-near (les graines Fibonacci et la palette logo officiel ne dépendent pas du backend WebGL).
3. Si bascule puppeteer, mettre à jour `scripts/package.json` : retirer `gl`, ajouter `puppeteer@^22`.

### 14.7 Métriques des 4 plans détaillés

| PR        | Plan                                               | Tasks   | Commits estimés | Lignes plan | Nouvelles deps                                                                                                                |
| --------- | -------------------------------------------------- | ------- | --------------- | ----------- | ----------------------------------------------------------------------------------------------------------------------------- |
| PR1       | `2026-04-27-ambient-lighting-v3-pr1-foundation.md` | 18      | 17              | 3278        | npm scripts: `three@0.169 + gl@8.0.2 (ou puppeteer@22) + canvas@2.11.2 + sharp@0.33.5 + vitest@1.6` ; package: deveps testing |
| PR2       | `2026-04-27-ambient-lighting-v3-pr2-web.md`        | 16      | 15              | —           | 0                                                                                                                             |
| PR3       | `2026-04-27-ambient-lighting-v3-pr3-mobile.md`     | 18      | ~14-16          | —           | 0                                                                                                                             |
| PR4       | `2026-04-27-ambient-lighting-v3-pr4-extension.md`  | 18      | ~17             | 2818        | Webpack plugin maison `critical-css-plugin.js` + `size-limit`                                                                 |
| **Total** |                                                    | **~70** | **~64**         |             |                                                                                                                               |

Ordre d'exécution validé : **PR1 foundation séquentiel** (bloque les 3 autres), puis **PR2 + PR3 + PR4 en parallèle** sur 3 worktrees distincts.
