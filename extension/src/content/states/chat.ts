...
function bindChatHandlers(
  ...
  onBack && $id("ds-chat-back")?.addEventListener("click", onBack);
...
function appendMessage(msg: ChatMessage): void {
  const container = $id("ds-chat-messages");
  if (!container) return;
...
function setInputDisabled(disabled: boolean): void {
  const input = $id<HTMLInputElement>("ds-chat-input");
  const btn = $id<HTMLButtonElement>("ds-chat-send");
