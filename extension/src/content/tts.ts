...
function updateProgressBar(): void {
  ...
  const bar = $id(`${activeButtonId}-progress-fill`);
  const time = $id(`${activeButtonId}-time`);
...
function bindActivePlayerEvents(
...
export function bindTTSButtons(): void {
  $qsa(".ds-tts-btn:not([data-bound])").forEach((el) => {
    const btn = el as HTMLElement;
...
export function bindTTSToolbar(): void {
  $id("ds-tts-lang")?.addEventListener("click", () => {
    state.language = state.language === "fr" ? "en" : "fr";
    saveSettings();
    const el = $id("ds-tts-lang");
    if (el) el.textContent = state.language === "fr" ? "🇫🇷" : "🇬🇧";
  });

  $id("ds-tts-gender")?.addEventListener("click", () => {
    state.gender = state.gender === "female" ? "male" : "female";
    saveSettings();
    const el = $id("ds-tts-gender");
    if (el) el.textContent = state.gender === "female" ? "♀" : "♂";
  });

  $id("ds-tts-speed-global")?.addEventListener("click", () => {
    ttsCycleSpeed();
    const el = $id("ds-tts-speed-global");
