...
describe('Token Storage', () => {
  ...
  it('clears auth data', async () => {
    ...
    await setStoredUser({ id: 1, username: 'test', email: 'test@test.com', plan: 'free', credits: 100, credits_monthly: 150 });
...
describe('User Storage', () => {
  ...
  it('stores and retrieves user', async () => {
    const mockUser = { id: 1, username: 'alice', email: 'alice@test.com', plan: 'pro' as const, credits: 500, credits_monthly: 1000 };
...
describe('Settings Storage', () => {
  ...
  it('stores and retrieves custom settings', async () => {
    const custom = { defaultMode: 'expert' as const, defaultLang: 'en' as const, showNotifications: false };
...
describe('Recent Analyses Storage', () => {
  ...
  it('adds a recent analysis with timestamp', async () => {
    ...
    await addRecentAnalysis({ videoId: 'vid-1', summaryId: 100, title: 'Test Video' });
    const analyses = await getRecentAnalyses();
  ...
  it('deduplicates by videoId (most recent first)', async () => {
    await addRecentAnalysis({ videoId: 'vid-1', summaryId: 100, title: 'Old Title' });
    await addRecentAnalysis({ videoId: 'vid-2', summaryId: 200, title: 'Other Video' });
    await addRecentAnalysis({ videoId: 'vid-1', summaryId: 101, title: 'New Title' });
  ...
  it('caps at 20 entries', async () => {
    for (let i = 0; i < 25; i++) {
      await addRecentAnalysis({ videoId: `vid-${i}`, summaryId: i, title: `Video ${i}` });
    }
    const analyses = await getRecentAnalyses();
