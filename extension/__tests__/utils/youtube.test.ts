...
import { extractVideoId as extractVideoIdLegacy, getThumbnailUrl as getThumbnailUrlLegacy } from '../../src/utils/youtube';
...
describe('extractYouTubeVideoId', () => {
  ...
  it('extracts ID from URL with extra params', () => {
    expect(extractYouTubeVideoId('https://www.youtube.com/watch?v=dQw4w9WgXcQ&t=120&list=PLx'))
      .toBe('dQw4w9WgXcQ');
  });
  ...
  it('extracts ID from short URL with timestamp', () => {
    expect(extractYouTubeVideoId('https://youtu.be/dQw4w9WgXcQ?t=42'))
      .toBe('dQw4w9WgXcQ');
  });
  ...
  it('extracts ID from shorts URL', () => {
    expect(extractYouTubeVideoId('https://www.youtube.com/shorts/abc123XYZ_-'))
      .toBe('abc123XYZ_-');
  });
  ...
  it('returns null for YouTube channel page', () => {
    expect(extractYouTubeVideoId('https://www.youtube.com/@MrBeast')).toBeNull();
  });
});
...
describe('extractTikTokVideoId', () => {
  it('extracts ID from standard TikTok URL', () => {
    expect(extractTikTokVideoId('https://www.tiktok.com/@user/video/7123456789012345678'))
      .toBe('7123456789012345678');
  });

  it('extracts ID from vm.tiktok short URL', () => {
    expect(extractTikTokVideoId('https://vm.tiktok.com/ZMrAbCdEf'))
      .toBe('ZMrAbCdEf');
  });

  it('extracts ID from m.tiktok URL', () => {
    expect(extractTikTokVideoId('https://m.tiktok.com/v/7123456789012345678'))
      .toBe('7123456789012345678');
  });

  it('extracts ID from tiktok.com/t/ URL', () => {
    expect(extractTikTokVideoId('https://www.tiktok.com/t/ZTR123abc'))
      .toBe('ZTR123abc');
  });

  it('returns null for non-TikTok URL', () => {
    expect(extractTikTokVideoId('https://www.youtube.com/watch?v=test')).toBeNull();
  });
...
describe('getYouTubeThumbnailUrl', () => {
  it('returns correct thumbnail URL', () => {
    expect(getYouTubeThumbnailUrl('dQw4w9WgXcQ'))
...
describe('getThumbnailUrl', () => {
  it('returns YouTube thumbnail for youtube platform', () => {
    expect(getThumbnailUrl('abc123', 'youtube'))
      .toBe('https://img.youtube.com/vi/abc123/default.jpg');
  });
...
describe('youtube.ts legacy — getThumbnailUrl', () => {
  it('returns correct URL', () => {
    expect(getThumbnailUrlLegacy('dQw4w9WgXcQ'))
      .toBe('https://img.youtube.com/vi/dQw4w9WgXcQ/default.jpg');
  });
});
