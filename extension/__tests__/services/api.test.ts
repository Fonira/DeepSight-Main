...
let handleMessage: (message: { action: string; data?: Record<string, unknown> }) => Promise<unknown>;

beforeAll(() => {
  // Import background.ts which registers the listener
  // The chrome-api-mock is already set up via setupFiles
  const onMessageAddListener = chrome.runtime.onMessage.addListener as jest.Mock;
...
describe('CHECK_AUTH', () => {
  ...
  it('returns authenticated: true with user when token exists', async () => {
    const mockUser = { id: 1, username: 'alice', email: 'alice@test.com', plan: 'free' };
    seedLocalStorage({
...
describe('LOGOUT', () => {
  it('clears auth and returns success', async () => {
    ...
    const response = await handleMessage({ action: 'LOGOUT' }) as { success: boolean };
    expect(response.success).toBe(true);
  });
});
...
describe('START_ANALYSIS', () => {
  it('starts analysis and returns task_id', async () => {
    ...
    const response = await handleMessage({
      ...
      data: { url: 'https://youtube.com/watch?v=test', options: { mode: 'standard', lang: 'fr' } },
...
describe('GET_PLAN', () => {
  it('returns plan info', async () => {
    ...
    const response = await handleMessage({ action: 'GET_PLAN' }) as { success: boolean; plan: typeof mockPlan };
...
describe('Token refresh on 401', () => {
  it('retries request after successful token refresh', async () => {
    ...
    const response = await handleMessage({ action: 'GET_PLAN' }) as { success: boolean; plan: { plan_id: string } };
...
describe('Unknown action', () => {
  it('returns error for unknown action', async () => {
    const response = await handleMessage({ action: 'UNKNOWN_ACTION' }) as { error: string };
