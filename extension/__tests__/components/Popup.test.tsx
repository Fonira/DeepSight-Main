...
jest.mock('../../src/i18n/useTranslation', () => ({
  useTranslation: () => ({
    t: {
      ...
      plans: { free: 'Gratuit', pro: 'Pro', starter: 'Standard', student: 'Starter' },
      upsell: {
      ...
      analysis: {
        ...
        mode: 'Mode',
        language: 'Langue',
        modes: { standard: 'Standard', accessible: 'Accessible', expert: 'Expert' },
      ...
      credits: { critical: 'Plus que {count} crédits', recharge: 'Recharger', remaining: '{count} crédits', low: 'cr.' },
...
describe('App — rendering', () => {
  it('renders without crashing', () => {
    ...
    (chrome.runtime.sendMessage as jest.Mock).mockResolvedValue({ authenticated: false });
  ...
  it('shows loading state initially', () => {
    ...
    (chrome.runtime.sendMessage as jest.Mock).mockReturnValue(new Promise(() => {}));
...
describe('App — unauthenticated state', () => {
  ...
  it('shows guest mode button', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Essayer sans compte (1 analyse gratuite)')).toBeInTheDocument();
    });
  });
...
describe('App — login flow', () => {
  it('transitions to main view on successful login', async () => {
    ...
    (chrome.runtime.sendMessage as jest.Mock)
      ...
      .mockResolvedValueOnce({ success: true, user: { id: 1, username: 'alice', email: 'a@b.com', plan: 'free', credits: 100, credits_monthly: 150 } })
      .mockResolvedValueOnce({ success: true, plan: { plan_name: 'Gratuit', plan_id: 'free', monthly_analyses: 3, analyses_this_month: 0, credits: 100, credits_monthly: 150, features: {} } });
    ...
    // Fill form
    await user.type(screen.getByPlaceholderText('Adresse e-mail'), 'alice@test.com');
    ...
    await waitFor(() => {
      ...
      expect(screen.queryByPlaceholderText('Adresse e-mail')).not.toBeInTheDocument();
    });
  });
  ...
  it('shows error on failed login', async () => {
    ...
    await user.type(screen.getByPlaceholderText('Adresse e-mail'), 'bad@test.com');
...
describe('App — authenticated state', () => {
  beforeEach(() => {
    const mockUser = { id: 1, username: 'alice', email: 'alice@test.com', plan: 'pro', credits: 12000, credits_monthly: 15000 };
    ...
    (chrome.tabs.query as jest.Mock).mockImplementation((_q: unknown, cb: (tabs: unknown[]) => void) => {
      cb([{ id: 1, url: 'https://www.youtube.com/watch?v=dQw4w9WgXcQ', title: 'Test Video' }]);
    });
  });
...
describe('App — guest mode', () => {
  it('enters guest mode when clicking guest button', async () => {
    ...
    (chrome.runtime.sendMessage as jest.Mock).mockResolvedValueOnce({ authenticated: false });

    // Mock tabs.query for video detection
    (chrome.tabs.query as jest.Mock).mockImplementation((_q: unknown, cb: (tabs: unknown[]) => void) => {
      cb([{ id: 1, url: 'https://www.youtube.com/watch?v=test123', title: 'Test' }]);
    });
    ...
    await user.click(screen.getByText('Essayer sans compte (1 analyse gratuite)'));
