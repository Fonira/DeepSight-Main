(() => {
        ...
        (function (e) {
          try {
            ...
            if (/(?:^|\.)youtube\.com$/.test(a.hostname)) {
              ...
              if (e) return e;
              const t = a.pathname.match(/^\/embed\/([^/?#]+)/);
              if (t) return t[1];
              const s = a.pathname.match(/^\/shorts\/([^/?#]+)/);
              if (s) return s[1];
            }
            if ("youtu.be" === a.hostname)
              return a.pathname.slice(1).split("/")[0] || null;
          } catch {
            return null;
          }
          return null;
  ...
  const t = {
    ...
    s = "ds_language";
  const i = "ds-voice-call-host";
  async function n() {
    if (document.getElementById(i)) return;
    const e = (function () {
      try {
        const e = new URL(location.href);
        return "/watch" !== e.pathname ? null : e.searchParams.get("v");
      } catch {
        return null;
      }
    })();
    ...
    const a = await (async function () {
      try {
        ...
        return e && e.success && e.state ? e.state : null;
      } catch {
        return null;
      }
    })();
    ...
    const n = (function () {
      ...
      { host: o, shadow: l } = (function (e) {
        ...
        const t = a.attachShadow({ mode: "closed" });
        return { host: a, shadow: t };
      })(r);
    (await (async function (e, a) {
      const i = await (async function () {
          try {
            const e = chrome.storage;
            if (!e?.sync?.get) return "fr";
            const a = (await e.sync.get(s))[s];
            return "fr" === a || "en" === a ? a : "fr";
          } catch {
            return "fr";
          }
        })(),
      ...
      ((r.className = "ds-voice-call-btn"),
        r.setAttribute("aria-label", n.buttonAriaLabel),
        (r.style.cssText = [
          "width:100%",
      ...
      const o = n.buttonLabelFloating;
      if (
        ((r.innerHTML = `<span>${o}</span>`),
        "free" !== a.plan ||
          a.trialUsed ||
          (r.innerHTML += `<span style="background:rgba(0,0,0,0.3);padding:2px 6px;border-radius:4px;font-size:10px">${n.trialBadge}</span>`),
        "free" === a.plan &&
          a.trialUsed &&
          ((r.disabled = !0),
          (r.style.opacity = "0.5"),
          (r.style.cursor = "not-allowed"),
          (r.title = n.trialUsedTitle),
          (r.innerHTML += `<span style="font-size:10px;opacity:0.7">${n.trialUsed}</span>`)),
        "expert" === a.plan || "pro" === a.plan)
      ) {
        const e = Math.max(0, 30 - (a.monthlyMinutesUsed ?? 0)),
          t = n.minutesRemaining.replace("{count}", String(e));
        r.innerHTML += `<span style="font-size:10px;opacity:0.7">${t}</span>`;
      }
      ...
      plan: a.plan,
      trialUsed: a.trialUsed,
      monthlyMinutesUsed: a.monthlyMinutesUsed,
      videoId: e,
      videoTitle: document.title,
    }),
      r ? document.body.appendChild(o) : n.insertBefore(o, n.firstChild));
  }
  const r = new (class {
    constructor() {
      ((this.originalVolume = null), (this.videoElement = null));
    }
    attach() {
      const e = document.querySelector("video");
      e &&
        ((this.videoElement = e),
        null === this.originalVolume && (this.originalVolume = e.volume),
        (e.volume = 0.1));
    }
    detach() {
      (this.videoElement &&
        null !== this.originalVolume &&
  ...
  chrome?.runtime?.onMessage?.addListener &&
    chrome.runtime.onMessage.addListener((e) => {
      ("DUCK_AUDIO" === e?.type && r.attach(),
        "RESTORE_AUDIO" === e?.type && r.detach());
    });
  let o = location.href;
  const l = (e) => {
      ((e) => {
        try {
          const a = new URL(e);
          return (
            /(?:^|\.)youtube\.com$/.test(a.hostname) && "/watch" === a.pathname
          );
        } catch {
          return !1;
        }
