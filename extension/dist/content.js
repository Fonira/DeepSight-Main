(() => {
  ...
  async function o() {
    ...
    (await (async function (e, a) {
      ...
      ((o.className = "ds-voice-call-btn"),
        o.setAttribute("aria-label", n.buttonAriaLabel),
        (o.style.cssText = [
          "width:100%",
      ...
      const r = n.buttonLabelFloating;
      if (
        ((o.innerHTML = `<span>${r}</span>`),
        "free" !== a.plan ||
          a.trialUsed ||
          (o.innerHTML += `<span style="background:rgba(0,0,0,0.3);padding:2px 6px;border-radius:4px;font-size:10px">${n.trialBadge}</span>`),
        "free" === a.plan &&
          a.trialUsed &&
          ((o.disabled = !0),
          (o.style.opacity = "0.5"),
          (o.style.cursor = "not-allowed"),
          (o.title = n.trialUsedTitle),
          (o.innerHTML += `<span style="font-size:10px;opacity:0.7">${n.trialUsed}</span>`)),
        "expert" === a.plan)
      ) {
        const e = Math.max(0, 30 - (a.monthlyMinutesUsed ?? 0)),
          t = n.minutesRemaining.replace("{count}", String(e));
        o.innerHTML += `<span style="font-size:10px;opacity:0.7">${t}</span>`;
      }
      ("pro" === a.plan &&
        (o.innerHTML += `<span style="font-size:10px;opacity:0.7">${n.upgradeBadge}</span>`),
      ...
      o ? document.body.appendChild(r) : t.insertBefore(r, t.firstChild));
  }
  const r = new (class {
    constructor() {
