...
test.describe("Page Load Performance", () => {
  for (const pageInfo of PAGES) {
    test(`${pageInfo.name} - measure load time & FCP`, async ({ page }) => {
      ...
      console.log(
        `PERF [${pageInfo.name}]: Total=${loadTime}ms, FCP=${metrics.fcp}ms, DCL=${metrics.domContentLoaded}ms, Load=${metrics.loadComplete}ms`,
      );
...
test.describe("Console Errors Per Page", () => {
  for (const pageInfo of PAGES) {
    test(`${pageInfo.name} - capture all console errors`, async ({ page }) => {
      ...
      warnings
        .slice(0, 5)
        .forEach((w, i) => console.log(`  WARN ${i + 1}: ${w.slice(0, 200)}`));
    });
  }
});
...
test.describe("Network Errors Per Page", () => {
  for (const pageInfo of PAGES) {
    test(`${pageInfo.name} - capture failed network requests`, async ({
      page,
    }) => {
...
test.describe("API Request Loop Detection", () => {
  test("dashboard should not make repeated identical API calls", async ({
    page,
  }) => {
...
test.describe("Missing Assets", () => {
  for (const pageInfo of PAGES) {
    test(`${pageInfo.name} - check for missing images/fonts/assets`, async ({
      page,
    }) => {
      ...
      page.on("response", (response) => {
        ...
        const isAsset =
          /\.(png|jpg|jpeg|gif|svg|woff|woff2|ttf|eot|ico|webp|avif)(\?|$)/i.test(
            url,
          );
...
test.describe("Bundle Size Check", () => {
  test("initial page load should transfer reasonable amount of data", async ({
    page,
  }) => {
    let totalTransfer = 0;
    ...
    resourceSizes
      ...
      .forEach((r) =>
        console.log(`  ${Math.round(r.size / 1024)}KB - ${r.url}`),
      );
  });
});
