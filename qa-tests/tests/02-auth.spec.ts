...
test.describe("Login Page", () => {
  ...
  test("should show error on invalid credentials", async ({ page }) => {
    ...
    const errorMsg = page.locator(
      '[role="alert"], .error, .text-red, [class*="error"], [class*="Error"], [class*="danger"]',
    );
  ...
  test("should have link to register/signup", async ({ page }) => {
    const registerLink = page.locator(
      'a:has-text("inscription"), a:has-text("register"), a:has-text("Créer"), a:has-text("Sign up"), button:has-text("inscription"), button:has-text("Créer un compte")',
    );
...
test.describe("Auth Token Management", () => {
  test("protected page without token should handle gracefully", async ({
    page,
  }) => {
    // Clear all storage
    await page.goto("/");
...
test.describe("Logout Flow", () => {
  test("after clearing token, protected pages should not be accessible", async ({
    page,
  }) => {
    await page.goto("/");
