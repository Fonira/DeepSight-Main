...
for (const viewport of VIEWPORTS) {
  test.describe(`${viewport.name} (${viewport.width}x${viewport.height})`, () => {
    for (const page_info of PAGES_TO_TEST) {
      test(`${page_info.name} should not have horizontal overflow`, async ({
        page,
      }) => {
        ...
        if (hasOverflow) {
          ...
          console.log(
            `OVERFLOW on ${page_info.name} at ${viewport.name}:`,
            overflowingElements,
          );
        }
        ...
        console.log(
          `${page_info.name} @ ${viewport.name}: overflow=${hasOverflow}`,
        );
      });

      test(`${page_info.name} should have readable text (not too small)`, async ({
        page,
      }) => {
        await page.setViewportSize({
          width: viewport.width,
        ...
        if (viewport.width <= 375) {
          const tooSmallText = await page.evaluate(() => {
            const small: string[] = [];
            document
              .querySelectorAll("p, span, a, li, td, th, label")
              .forEach((el) => {
                ...
                if (
                  ...
                  small.push(
                    `${el.tagName}: "${el.textContent.trim().slice(0, 30)}" (${fontSize}px)`,
                  );
                }
              });
          ...
          if (tooSmallText.length > 0) {
            console.log(
              `Small text on ${page_info.name} mobile:`,
              tooSmallText,
            );
          }
        }
...
test.describe("Mobile Navigation", () => {
  test("should have mobile menu/hamburger on small screens", async ({
    page,
  }) => {
  ...
  test("sidebar should be hidden on mobile", async ({ page }) => {
    ...
    const sidebar = page.locator(
      '[class*="sidebar" i], aside, nav[class*="side" i]',
    );
    ...
    console.log(
      `Visible sidebars on mobile: ${visibleSidebars.length}`,
      visibleSidebars,
    );
  });
});
  ...
  test("modals should be dismissible with Escape key", async ({ page }) => {
    ...
    if ((await modalTriggers.count()) > 0) {
      ...
      const modal = page.locator(
        '[role="dialog"], [class*="modal" i], [class*="Modal" i], [class*="overlay" i]',
      );
      if ((await modal.count()) > 0) {
        ...
        const modalAfter = page.locator(
          '[role="dialog"]:visible, [class*="modal" i]:visible',
        );
