...
test.describe("Double Click Prevention", () => {
  test("double click on submit should not trigger double request", async ({
    page,
  }) => {
    ...
    if (await input.isVisible().catch(() => false)) {
      ...
      if (await submitBtn.isVisible().catch(() => false)) {
        ...
        const analyzeRequests = apiRequests.filter((r) =>
          r.includes("analyze"),
        );
        console.log(
          `Analyze requests after double-click: ${analyzeRequests.length}`,
        );
        if (analyzeRequests.length > 1) {
          console.log(
            "WARNING: Double submit detected! Requests:",
            analyzeRequests,
          );
        }
      }
...
test.describe("Special Characters in Inputs", () => {
  test("special characters in URL input should not break the UI", async ({
    page,
  }) => {
    await page.goto("/dashboard", { waitUntil: "networkidle" });
