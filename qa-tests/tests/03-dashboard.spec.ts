...
test.describe("Dashboard Page", () => {
  ...
  test("should show loading states, not infinite skeletons", async ({
    page,
  }) => {
    ...
    const skeletons = page.locator(
      '.animate-pulse, [class*="skeleton"], [class*="Skeleton"]',
    );
  ...
  test("empty URL submission should show validation error", async ({
    page,
  }) => {
...
test.describe("Video Analysis Flow", () => {
  test("submitting a YouTube URL should trigger analysis or show appropriate message", async ({
    page,
  }) => {
    await page.goto("/dashboard", { waitUntil: "networkidle" });
    ...
    if (await input.isVisible().catch(() => false)) {
      ...
      console.log(
        `After YouTube URL submit - Loading: ${hasLoading}, Error: ${hasError}`,
      );
