  ...
  for (const route of PUBLIC_ROUTES) {
    test(`${route.name} (${route.path}) should render content`, async ({
      ...
      const errorOverlay = page.locator(
        "#webpack-dev-server-client-overlay, .error-overlay, [data-reactroot]",
      );
...
test.describe("Protected Routes - Auth Redirect", () => {
  for (const route of PROTECTED_ROUTES) {
    test(`${route.name} (${route.path}) without auth should redirect or show content (dev bypass)`, async ({
      page,
    }) => {
      const consoleErrors: string[] = [];
      page.on("console", (msg) => {
...
test.describe("Navigation Links", () => {
  test("Landing page should have navigation to login", async ({ page }) => {
    ...
    const loginLink = page.locator(
      'a[href*="login"], button:has-text("Connexion"), button:has-text("Login"), a:has-text("Connexion"), a:has-text("Login")',
    );
