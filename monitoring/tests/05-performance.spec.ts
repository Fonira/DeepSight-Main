...
test.describe("Performance @smoke @api", () => {
  ...
  test("pas de mixed content — les assets chargent en HTTPS", async ({
    page,
  }) => {
  ...
  test("les assets critiques (CSS, JS) chargent avec succès", async ({
    page,
  }) => {
    ...
    page.on("response", (res) => {
      ...
      if ((url.endsWith(".js") || url.endsWith(".css")) && status >= 400) {
