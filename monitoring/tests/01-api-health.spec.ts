...
test.describe('API Health @smoke @api', () => {
  ...
  test('GET /api/billing/plans retourne la liste des plans', async ({ request }) => {
  ...
  test('POST /api/auth/login avec mauvais creds retourne une erreur', async ({ request }) => {
    const res = await request.post(`${API_URL}${API_ENDPOINTS.login}`, {
      headers: { 'Content-Type': 'application/json' },
      data: {
  ...
  test('GET /api/tournesol/recommendations répond sans 500', async ({ request }) => {
