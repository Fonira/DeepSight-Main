...
import { detectReducedMotion, detectHighContrast, getReadingZoneCap, } from "./accessibility.js";
/**
 * Find the two keyframes surrounding a given hour, with wrap-around.
 * Returns { from, to, factor } where factor is in [0, 1).
 */
export function findKeyframePair(hour) {
 ...
 */
function blendBeamType(from, to, factor) {
    if (from === to)
        return from;
 ...
 */
export function getAmbientPreset(date = new Date(), opts = {}) {
    ...
    const beamColor = lerpColor(from.beamColor, to.beamColor, factor);
    const baseAngle = lerpAngle(from.beamAngleDeg, to.beamAngleDeg, factor);
    const beamAngleDeg = (baseAngle + angleVar + 360) % 360;
    const beamOpacity = clamp(lerp(from.beamOpacity, to.beamOpacity, factor) * intensityMul);
    // Discs — use trajectory functions for smooth motion
    const sunState = getSunState(hour);
    const moonState = getMoonState(hour);
    // Ambient layers
    const ambientPrimary = clamp(lerp(from.ambientPrimary, to.ambientPrimary, factor) * intensityMul);
    const ambientSecondary = clamp(lerp(from.ambientSecondary, to.ambientSecondary, factor) * intensityMul);
    const ambientTertiary = clamp(lerp(from.ambientTertiary, to.ambientTertiary, factor) * intensityMul);
    // Stars
    const starOpacityMul = lerp(from.starOpacityMul, to.starOpacityMul, factor) * intensityMul;
    ...
    const secondary = lerpColor(from.colors.secondary, to.colors.secondary, factor);
 ...
 */
function findKeyframeV3Pair(hour) {
    const exact = KEYFRAMES_V3.findIndex((k) => k.hour === hour);
    if (exact !== -1)
        return { fromIdx: exact, toIdx: exact, factor: 0 };
 ...
 */
export function getAmbientPresetV3(date, opts = {}) {
    ...
    const opacity = lerp(from.beamOpacity, to.beamOpacity, f) * (opts.intensityMul ?? 1);
    const intensity = lerp(from.intensity, to.intensity, f) * (opts.intensityMul ?? 1);
    ...
    const nightMode = opts.forceNightMode !== undefined ? opts.forceNightMode : computedNightMode;
    // haloAccentColor : si l'une des 2 keyframes en a, garder celle de la plus proche
    const haloAccentColor = factor < 0.5 ? from.haloAccentColor : to.haloAccentColor;
    const skipCss = opts.skipCssStrings ?? false;
    return {
        hour: totalHour,
