 ...
 */
export const SUNFLOWER_PALETTES = {
    dawn: {
        petalOuter: "#FFB347",
        petalInner: "#FFD580",
        core: "#A0522D",
...
export const SUNFLOWER_PETAL_SCALE = {
    dawn: 0.88,
    day: 1.0,
    dusk: 0.88,
    night: 0.6,
};
export const SUNFLOWER_HALOS = {
    dawn: {
        gradient: "radial-gradient(circle, rgba(255,179,71,0.32) 0%, rgba(255,179,71,0.1) 40%, transparent 70%)",
        pulse: false,
    },
    day: { gradient: "transparent", pulse: false },
    dusk: {
        gradient: "radial-gradient(circle, rgba(255,140,66,0.32) 0%, rgba(244,114,182,0.18) 40%, transparent 70%)",
        pulse: false,
    },
    night: {
        gradient: "radial-gradient(circle, rgba(139,92,246,0.45) 0%, rgba(99,102,241,0.28) 35%, rgba(99,102,241,0.08) 60%, transparent 80%)",
        pulse: true,
    },
};
/**
 ...
 *   20h-4h59 → night
 */
export function getSunflowerPhase(frameIndex) {
 ...
 */
export function getSunflowerRotation(frameIndex) {
    const h = ((frameIndex % 24) + 24) % 24;
    if (h < 5 || h >= 20)
        return 175;
    if (h < 7) {
...
export function getSunflowerOpacity(frameIndex) {
    const h = ((frameIndex % 24) + 24) % 24;
    const phase = getSunflowerPhase(h);
    if (phase === "night")
        return 0.55;
    if (phase === "dawn")
        return 0.6 + ((h - 5) / 2) * 0.3;
    if (phase === "dusk")
        return 0.9 - ((h - 17) / 3) * 0.4;
