...
describe('computeAngleVariation', () => {
  ...
  it('is deterministic for same (seed, hour)', () => {
    expect(computeAngleVariation(99, 14.5)).toBe(computeAngleVariation(99, 14.5));
  });

  it('different seeds give different variations', () => {
    expect(computeAngleVariation(1, 12)).not.toBe(computeAngleVariation(999, 12));
  });
...
describe('getAmbientPreset integration', () => {
  ...
  it('skipCssStrings returns empty cssColor', () => {
    const p = getAmbientPreset(new Date(2026, 5, 21, 12), { skipCssStrings: true });
